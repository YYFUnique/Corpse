...
CHardNotify::CHardNotify()
{
	AddVirtualWnd(_T("hard"),&m_Hardware);
}
...
DUI_BEGIN_MESSAGE_MAP(CHardNotify, CNotifyPump)
	DUI_ON_MSGTYPE(DUI_MSGTYPE_SELECTCHANGED,OnSelectChanged)
DUI_END_MESSAGE_MAP()

//void CHardNotify::SetPaintMagager(CPaintManagerUI* pPaintMgr)
//{
//	CVirtualWnd::SetPaintMagager(pPaintMgr);
//	CBase::SetPaintManager(m_pPaintManager);
//}

void CHardNotify::OnSelectChanged(TNotifyUI& msg)
{
	if (msg.pSender->GetParent()->GetName() == _T("VLayoutNotify"))
	{
...
BOOL CHardNotify::DeviceChanged(WPARAM wParam, LPARAM lParam)
{
	if (wParam == DBT_DEVICEREMOVECOMPLETE || wParam == DBT_DEVICEARRIVAL )
	{
		...
		if (lpdb->dbch_devicetype == DBT_DEVTYP_DEVICEINTERFACE || 
			...
			return m_Hardware.DeviceChanged(wParam, lParam);
	}

	return TRUE;	
}

void CHardNotify::GetDeviceChangeEventTypeText(UINT nEventType,LPTSTR lpszEventTypeText)
{
...
void CHardNotify::ConvertGuidToHardwareType(REFGUID rguid, LPWSTR lpwszHardwareType)
{
...
void CHardNotify::GetDeviceEventText(DWORD dwType, DWORD dwData, LPTSTR szEventText)
{
	szEventText[0]=0;
	switch(dwType)
	{
...
void CHardNotify::OnConfigChangeCanceled(DWORD dwData,LPTSTR szEventText)
{
	_stprintf_s(szEventText,100,_T("lParam=%08X"),dwData);
}

void CHardNotify::OnDeviceArrival(DWORD dwData,LPTSTR szEventText)
{
	GetDevBroadcastInfo((DEV_BROADCAST_HDR*)dwData, szEventText);
}

void CHardNotify::OnDeviceRemoveComplete(DWORD dwData,LPTSTR szEventText)
{
	GetDevBroadcastInfo((DEV_BROADCAST_HDR*)dwData, szEventText);
}

void CHardNotify::OnDeviceRemovePending(DWORD dwData,LPTSTR szEventText)
{
	GetDevBroadcastInfo((DEV_BROADCAST_HDR*)dwData, szEventText);
}

void CHardNotify::OnDeviceQueryRemove(DWORD dwData,LPTSTR szEventText)
{
	GetDevBroadcastInfo((DEV_BROADCAST_HDR*)dwData, szEventText);
}

void CHardNotify::GetDevBroadcastInfo(PDEV_BROADCAST_HDR pDevBroadcastHdr,LPTSTR lpszEventText)
{
