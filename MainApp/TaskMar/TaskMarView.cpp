...
IMPLEMENT_DYNCREATE(CTaskMarView, CListView)

BEGIN_MESSAGE_MAP(CTaskMarView, CListView)
...
void CTaskMarView::OnInitialUpdate()
{
	...
	//pMainFrame->SetTaskbarOverlayIcon()
	...
	ListCtrl.SetExtendedStyle(LVS_EX_SINGLEROW|LVS_EX_LABELTIP|LVS_EX_FULLROWSELECT|LVS_EX_DOUBLEBUFFER);
	ListCtrl.SetImageList(&m_ImgList,LVSIL_SMALL);

	UINT uIndex=0;
	ListCtrl.InsertColumn(uIndex++,_T("进程名称"),LVCFMT_LEFT,157);
	ListCtrl.InsertColumn(uIndex++,_T("进程ID"),LVCFMT_RIGHT,58);
	ListCtrl.InsertColumn(uIndex++,_T("进程路径"),LVCFMT_LEFT,231);
	ListCtrl.InsertColumn(uIndex++,_T("用户名"),LVCFMT_RIGHT,130);
	ListCtrl.InsertColumn(uIndex++,_T("内存使用"),LVCFMT_RIGHT,70);
	ListCtrl.InsertColumn(uIndex++,_T("CPU使用"),LVCFMT_RIGHT,70);
	ListCtrl.InsertColumn(uIndex++,_T("线程数"),LVCFMT_RIGHT,50);
	ListCtrl.InsertColumn(uIndex++,_T("句柄数"),LVCFMT_RIGHT,50);
	ListCtrl.InsertColumn(uIndex++,_T("会话ID"),LVCFMT_RIGHT,50);
	ListCtrl.InsertColumn(uIndex++,_T("文件描述"),LVCFMT_LEFT,250);
...
UINT CTaskMarView::RefreshProcessListThread(LPVOID lParam)
{
	...
	while(TRUE)
	{
		...
		ListCtrl.RedrawItems(0,ListCtrl.GetItemCount()-1);
...
void CTaskMarView::InsertProcessItem(LS_SYSTEM_PROCESS& LsSystemProcess,ITEM_STATE ItemState/* = ITEM_STATE_Normal*/)
{
	...
	int nIndex = ListCtrl.InsertItem(LVIF_IMAGE|LVIF_TEXT|LVIF_PARAM,0x7FFFFFFF,LsSystemProcess.strProcessName,0,0,nIco,(LPARAM)Pos);

	CString strPID;
	int nSubItem = 0;
	strPID.Format(_T("%d"),LsSystemProcess.dwProcessId);
	ListCtrl.SetItemText(nIndex,++nSubItem,strPID);	
	ListCtrl.SetItemText(nIndex,++nSubItem,LsSystemProcess.strProcessPath);
	ListCtrl.SetItemText(nIndex,++nSubItem,LsSystemProcess.strUserName);
	ListCtrl.SetItemText(nIndex,++nSubItem,FormatNumSizeToStringSize(LsSystemProcess.dwMemUse));
	ListCtrl.SetItemText(nIndex,++nSubItem,FormatNumToString(0));
	ListCtrl.SetItemText(nIndex,++nSubItem,FormatNumToString(LsSystemProcess.dwThread));
	ListCtrl.SetItemText(nIndex,++nSubItem,FormatNumToString(LsSystemProcess.dwHandle));
	ListCtrl.SetItemText(nIndex,++nSubItem,FormatNumToString(LsSystemProcess.dwConsole));
	ListCtrl.SetItemText(nIndex,++nSubItem,LsSystemProcess.strDescrible);
...
void CTaskMarView::DeleteItem(UINT nItem)
{
	...
	if (pos)
	{
		...
		ListCtrl.SetItemData(nItem,(DWORD_PTR)pos);
	}	
}
...
void CTaskMarView::ModifyItemData()
{
	...
	for (int i=ListCtrl.GetItemCount()-1;i>=0;--i)
	{
		...
		if (pos)
		{
			...
			{
				ItemData.ItemState = ITEM_STATE_Normal;
				ListCtrl.SetItemData(i,(DWORD_PTR)pos);
			}
		}
	}
}
...
void CTaskMarView::OnCopyProcessName()
{
	...
	CopyDataToClipboard(CF_UNICODETEXT,m_hWnd,strProcessName);
}
