...
*/
  ...
  rc = ldap_search_sA(server, ludp->lud_dn, ludp->lud_scope,
  ...
  for(num = 0, entryIterator = ldap_first_entry(server, result);
    ...
    char  *dn = ldap_get_dnA(server, entryIterator);
    int i;
    ...
    for(attribute = ldap_first_attributeA(server, entryIterator, &ber);
        attribute;
        attribute = ldap_next_attributeA(server, entryIterator, ber)) {
      BerValue **vals = ldap_get_values_lenA(server, entryIterator, attribute);
