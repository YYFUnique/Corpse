...
void CHash::GetHashHexText(const BYTE* pHashBuffer,LPTSTR szHashText)
{
	...
	{
		_stprintf_s(szHashText+2*i, 2, _T("%02X"), pHashBuffer[i]);
	}
}

void CHash::GetHashHexText(const BYTE* pHashBuffer,CString& szHashText)
{
	TCHAR szHashValue[MAX_PATH];
	GetHashHexText(pHashBuffer, szHashValue);
	szHashText = szHashValue;
}
