...
void ILibThreadPool_AddThread(ILibThreadPool pool)
{
	...
	do
	{
		...
		if(wi != NULL && !abort)
		{
			...
			__except(1,1)
			{
				sprintf(sz, "DLNA: wi is invalid! (wi=%08x)", (int)wi);
				OutputDebugStringA(sz);
			}

			sprintf(sz, "DLNA: free wi 0! (wi=%08x)", (int)wi);
			OutputDebugStringA(sz);
		...
		{
			char sz[100];
			sprintf(sz, "DLNA: free wi 1! (wi=%08x)", (int)wi);
			OutputDebugStringA(sz);
...
void ILibThreadPool_QueueUserWorkItem(ILibThreadPool pool, void *var, ILibThreadPool_Handler callback)
{
	...
	if(NumThreads==0)
	{
		...
		//
		char sz[100];
		sprintf(sz, "DLNA: free wi 2! (wi=%08x)", (int)wi);
		OutputDebugStringA(sz);
