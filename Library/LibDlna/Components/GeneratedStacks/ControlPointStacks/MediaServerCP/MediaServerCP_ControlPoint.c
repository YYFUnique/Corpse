...
#		include "stdafx.h"
#	endif
char* MediaServerCP_PLATFORM = "WINDOWS UPnP/1.0 Intel MicroStack/1.0.2628";
#elif defined(__SYMBIAN32__)
char* MediaServerCP_PLATFORM = "SYMBIAN UPnP/1.0 Intel MicroStack/1.0.2628";
#else
char* MediaServerCP_PLATFORM = "POSIX UPnP/1.0 Intel MicroStack/1.0.2628";
#endif
...
#define MediaServerCP_MIN(a,b) (((a)<(b))?(a):(b))
...
void MediaServerCP_Renew(void *state);
void MediaServerCP_SSDP_Sink(void *sender, char* UDN, int Alive, char* LocationURL, int Timeout,UPnPSSDP_MESSAGE m, void *cp);
...
void (*MediaServerCP_EventCallback_ConnectionManager_SourceProtocolInfo)(struct UPnPService* Service,char* value);
void (*MediaServerCP_EventCallback_ConnectionManager_SinkProtocolInfo)(struct UPnPService* Service,char* value);
void (*MediaServerCP_EventCallback_ConnectionManager_CurrentConnectionIDs)(struct UPnPService* Service,char* value);
void (*MediaServerCP_EventCallback_ContentDirectory_ContainerUpdateIDs)(struct UPnPService* Service,char* value);
void (*MediaServerCP_EventCallback_ContentDirectory_SystemUpdateID)(struct UPnPService* Service,unsigned int value);
...
struct MediaServerCP__Stack
{
	void *data;
	struct MediaServerCP__Stack *next;
};


void MediaServerCP_SetUser(void *token, void *user)
{
	((struct MediaServerCP_CP*)token)->User = user;
}
void* MediaServerCP_GetUser(void *token)
{
	return(((struct MediaServerCP_CP*)token)->User);
}
void MediaServerCP_CP_ProcessDeviceRemoval(struct MediaServerCP_CP* CP, struct UPnPDevice *device);



void MediaServerCP_DestructUPnPService(struct UPnPService *service)
{
	...
	{
		ILibLifeTime_Remove(((struct MediaServerCP_CP*)service->Parent->CP)->LifeTimeMonitor,service);
		ILibDeleteEntry(((struct MediaServerCP_CP*)service->Parent->CP)->SIDTable,service->SubscriptionID,(int)strlen(service->SubscriptionID));
...
void MediaServerCP_DestructUPnPDevice(struct UPnPDevice *device)
{
	...
	{
		d2 = d1->Next;
		MediaServerCP_DestructUPnPDevice(d1);
		d1 = d2;
	}
	...
	{
		s2 = s1->Next;
		MediaServerCP_DestructUPnPService(s1);
		s1 = s2;
	}
...
/*! \fn MediaServerCP_AddRef(struct UPnPDevice *device)
...
void MediaServerCP_CheckfpDestroy(struct UPnPDevice *device)
{
	...
	while(ed!=NULL)
	{
		MediaServerCP_CheckfpDestroy(ed);
		ed = ed->Next;
	}
}
/*! \fn MediaServerCP_Release(struct UPnPDevice *device)
...
void MediaServerCP_Push(struct MediaServerCP__Stack **pp_Top, void *data)
{
	struct MediaServerCP__Stack *frame = (struct MediaServerCP__Stack*)malloc(sizeof(struct MediaServerCP__Stack));
	frame->data = data;
	frame->next = *pp_Top;
	*pp_Top = frame;
}
void *MediaServerCP_Pop(struct MediaServerCP__Stack **pp_Top)
{
	struct MediaServerCP__Stack *frame = *pp_Top;
	void *RetVal = NULL;
...
void *MediaServerCP_Peek(struct MediaServerCP__Stack **pp_Top)
{
	struct MediaServerCP__Stack *frame = *pp_Top;
	void *RetVal = NULL;
...
void MediaServerCP_Flush(struct MediaServerCP__Stack **pp_Top)
{
	while(MediaServerCP_Pop(pp_Top)!=NULL) {}
	*pp_Top = NULL;
}

/*! \fn MediaServerCP_GetDeviceAtUDN(void *v_CP,char* UDN)
...
struct UPnPDevice* MediaServerCP_GetDeviceAtUDN(void *v_CP,char* UDN)
{
	...
	struct MediaServerCP_CP* CP = (struct MediaServerCP_CP*)v_CP;
...
struct packetheader *MediaServerCP_BuildPacket(char* IP, int Port, char* Path, char* cmd)
{
	...
	ILibAddHeaderLine(RetVal,"User-Agent",10,MediaServerCP_PLATFORM,(int)strlen(MediaServerCP_PLATFORM));
	free(HostLine);
	return(RetVal);
}

void MediaServerCP_RemoveServiceFromDevice(struct UPnPDevice *device, struct UPnPService *service)
{
	...
	if(s==service)
	{
		...
		MediaServerCP_DestructUPnPService(service);
		return;
	}
	while(s->Next!=NULL)
	{
...
void MediaServerCP_ProcessDevice(void *v_cp,struct UPnPDevice *device)
{
	struct MediaServerCP_CP* cp = (struct MediaServerCP_CP*)v_cp;
	...
	{
		MediaServerCP_ProcessDevice(v_cp,EmbeddedDevice);
...
/*! fn MediaServerCP_PrintUPnPDevice(int indents, struct UPnPDevice *device)
...
*/
void MediaServerCP_PrintUPnPDevice(int indents, struct UPnPDevice *device)
{
	...
	while(d!=NULL)
	{
		MediaServerCP_PrintUPnPDevice(indents+5,d);
		d = d->Next;
	}
}

/*! \fn MediaServerCP_GetService(struct UPnPDevice *device, char* ServiceName, int length)
...
{
	return(MediaServerCP_GetService(device,"urn:schemas-upnp-org:service:ContentDirectory:1",47));
}


struct UPnPDevice *MediaServerCP_GetDevice2(struct UPnPDevice *device, int index, int *c_Index)
{
	struct UPnPDevice *RetVal = NULL;
	...
	while(e_Device!=NULL)
	{
		RetVal = MediaServerCP_GetDevice2(e_Device,index,&currentIndex);
		if(RetVal!=NULL)
		{
			break;
		}
...
{
	int c_Index = -1;
	return(MediaServerCP_GetDevice2(device,index,&c_Index));
}
int MediaServerCP_GetDeviceCount(struct UPnPDevice *device)
{
	...
	while(e_Device!=NULL)
	{
		RetVal += MediaServerCP_GetDeviceCount(e_Device);
		e_Device = e_Device->Next;
	}
...
//
int MediaServerCP_GetErrorCode(char *buffer, int length)
{
	int RetVal = 500;
							...
							if(xml->NameLength==5 && memcmp(xml->Name,"Fault",5)==0)
							{
								...
								while(xml!=NULL)
								{
									if(xml->NameLength==6 && memcmp(xml->Name,"detail",6)==0)
									{
										...
										while(xml!=NULL)
										{
											if(xml->NameLength==9 && memcmp(xml->Name,"MediaServerCP_Error",9)==0)
											{
												xml = xml->Next;
												while(xml!=NULL)
												{
...
//
void MediaServerCP_ProcessSCPD(char* buffer, int length, struct UPnPService *service)
{
...
//
void MediaServerCP_InterfaceChanged(struct UPnPDevice *device)
{
	void *cp = device->CP;
	char *UDN;
	char *LocationURL;
	...
	MediaServerCP_SSDP_Sink(NULL, device->UDN, 0, NULL, 0,UPnPSSDP_NOTIFY,device->CP);
	MediaServerCP_SSDP_Sink(NULL, UDN, -1, LocationURL, Timeout, UPnPSSDP_NOTIFY,cp);
...
//
void MediaServerCP_ExpiredDevice(struct UPnPDevice *device)
{
...
//
void MediaServerCP_FinishProcessingDevice(struct MediaServerCP_CP* CP, struct UPnPDevice *RootDevice)
{
	...
	do
	{
		RetDevice = MediaServerCP_GetDevice1(RootDevice,i++);
		if(RetDevice!=NULL)
		{
			//
			...
			MediaServerCP_AddRef(RetDevice);
	...
	//
	ILibLifeTime_Add(CP->LifeTimeMonitor,RootDevice,Timeout,(void*)&MediaServerCP_ExpiredDevice,NULL);
}

//
// Internal HTTP Sink for fetching the SCPD document
//
void MediaServerCP_SCPD_Sink(
void *WebReaderToken,
int IsInterrupt,
...
int *PAUSE)
{
	...
	struct MediaServerCP_CP *CP = service->Parent->CP;
	...
	{
		MediaServerCP_ProcessSCPD(buffer,EndPointer, service);
		...
		if(device->SCPDLeft==0)
		{
			if(device->SCPDError==0)
			{
				...
				//
				MediaServerCP_FinishProcessingDevice(CP,device);
			}
			else if(IsInterrupt==0)
			{
				...
				// stale device
				//
				MediaServerCP_CP_ProcessDeviceRemoval(CP,device);
				MediaServerCP_DestructUPnPDevice(device);
			}
		}
	}
	else
...
//
void MediaServerCP_CalculateSCPD_FetchCount(struct UPnPDevice *device)
{
	int count = 0;
	...
	while(e_Device!=NULL)
	{
		MediaServerCP_CalculateSCPD_FetchCount(e_Device);
		e_Device = e_Device->Next;
	}
...
//
void MediaServerCP_SCPD_Fetch(struct UPnPDevice *device)
{
	struct UPnPDevice *e_Device = device->EmbeddedDevices;
	struct UPnPService *s;
	char *IP,*Path;
	...
	while(e_Device!=NULL)
	{
		...
		//
		MediaServerCP_SCPD_Fetch(e_Device);
		e_Device = e_Device->Next;
	}
	...
	while(s!=NULL)
	{
		...
		p = MediaServerCP_BuildPacket(IP,Port,Path,"GET");
		...
		((struct MediaServerCP_CP*)device->CP)->HTTP,
		&addr,
		p,
		&MediaServerCP_SCPD_Sink,
		device,
		s);
...
/*! \fn MediaServerCP_GetCustomTagFromDevice(struct UPnPDevice *d, char* FullNameSpace, char* Name)
...
void MediaServerCP_ProcessDeviceXML_iconList(struct UPnPDevice *device,const char *BaseURL,struct ILibXMLNode *xml)
{
...
struct UPnPDevice* MediaServerCP_ProcessDeviceXML_device(struct ILibXMLNode *xml, void *v_CP,const char *BaseURL, int Timeout, int RecvAddr)
{
	...
	while(xml!=NULL)
	{
			...
			if(xml->Next->StartTag!=0)
			{
				...
				while(flg2==0)
				{
					if(xml->NameLength==6 && memcmp(xml->Name,"device",6)==0)
					{
						...
						//
						tempDevice = MediaServerCP_ProcessDeviceXML_device(xml,v_CP,BaseURL,Timeout, RecvAddr);
		...
		{
			MediaServerCP_ProcessDeviceXML_iconList(device,BaseURL,xml->Next);
		} else
...
//
int MediaServerCP_ProcessDeviceXML(void *v_CP,char* buffer, int BufferSize, char* LocationURL, int RecvAddr, int Timeout)
{
	...
	//
	RootDevice = MediaServerCP_ProcessDeviceXML_device(xml,v_CP,BaseURL,Timeout,RecvAddr);
	free(BaseURL);
	ILibDestructXMLNodeList(rootXML);
	...
	//
	MediaServerCP_ProcessDevice(v_CP,RootDevice);
	RootDevice->SCPDLeft = 0;
	MediaServerCP_CalculateSCPD_FetchCount(RootDevice);
	if(RootDevice->SCPDLeft==0)
	{
		...
		//
		MediaServerCP_FinishProcessingDevice(v_CP,RootDevice);
	}
	else
	{
		//
		...
		//
		MediaServerCP_SCPD_Fetch(RootDevice);
	}
	return(0);
}
...
//
void MediaServerCP_HTTP_Sink_DeviceDescription(
void *WebReaderToken,
int IsInterrupt,
struct packetheader *header,
char *buffer,
...
int *PAUSE)
{
	...
	struct MediaServerCP_CP* CP = (struct MediaServerCP_CP*)cp;
	...
	{
		if(MediaServerCP_ProcessDeviceXML(cp,buffer,EndPointer-(*p_BeginPointer),customData->buffer,header->ReceivingAddress,customData->Timeout)!=0)
		{
...
void MediaServerCP__FlushRequest(struct UPnPDevice *device)
{
	struct UPnPDevice *ed = device->EmbeddedDevices;
	struct UPnPService *s = device->Services;
	...
	while(ed!=NULL)
	{
		MediaServerCP__FlushRequest(ed);
		ed = ed->Next;
	}
	while(s!=NULL)
	{
...
//
void MediaServerCP_CP_RecursiveReleaseAndEventDevice(struct MediaServerCP_CP* CP, struct UPnPDevice *device)
{
	...
	while(temp!=NULL)
	{
		MediaServerCP_CP_RecursiveReleaseAndEventDevice(CP,temp);
		temp = temp->Next;
	}
...
void MediaServerCP_CP_ProcessDeviceRemoval(struct MediaServerCP_CP* CP, struct UPnPDevice *device)
{
	...
	while(temp!=NULL)
	{
		MediaServerCP_CP_ProcessDeviceRemoval(CP,temp);
		temp = temp->Next;
	}
	...
	while(s!=NULL)
	{
		...
		ILibWebClient_DeleteRequests(((struct MediaServerCP_CP*)device->CP)->HTTP,IP,(int)port);
		free(IP);
		free(PATH);
...
void MediaServerCP_SSDP_Sink(void *sender, char* UDN, int Alive, char* LocationURL, int Timeout,UPnPSSDP_MESSAGE m,void *cp)
{
	...
	if(Alive!=0)
	{
		...
		if(ILibHasEntry(CP->DeviceTable_URI,LocationURL,(int)strlen(LocationURL))==0 && ILibHasEntry(CP->DeviceTable_UDN,UDN,(int)strlen(UDN))==0)
		{
			...
			((struct MediaServerCP_CP*)cp)->HTTP,
			&addr,
			p,
			&MediaServerCP_HTTP_Sink_DeviceDescription,
			customData, 
			cp);
		...
		else
		{
			...
			if(device!=NULL && device->ReservedID==0 && m==UPnPSSDP_NOTIFY)
			{
				...
				if(strcmp(device->LocationURL,LocationURL)==0)
				{
					...
					ILibLifeTime_Remove(((struct MediaServerCP_CP*)cp)->LifeTimeMonitor,device);
					ILibLifeTime_Add(((struct MediaServerCP_CP*)cp)->LifeTimeMonitor,device,Timeout,(void*)&MediaServerCP_ExpiredDevice,NULL);
				}
				else
				{
					//
					...
					if(t.tv_sec-device->Reserved2>10)
					{
						...
						ILibLifeTime_Remove(((struct MediaServerCP_CP*)cp)->LifeTimeMonitor,device);
						ILibLifeTime_Add(((struct MediaServerCP_CP*)cp)->LifeTimeMonitor,device,7,(void*)&MediaServerCP_InterfaceChanged,NULL);
					}
				}
			}
		}
...
void MediaServerCP_ConnectionManager_EventSink(char* buffer, int bufferlength, struct UPnPService *service)
{
	struct ILibXMLNode *xml,*rootXML;
	char *tempString;
	int tempStringLength;
					...
					if(xml->NameLength==8 && memcmp(xml->Name,"property",8)==0)
					{
						...
						while(flg2==0)
						{
							if(xml->NameLength==18 && memcmp(xml->Name,"SourceProtocolInfo",18) == 0)
							{
								...
								if(MediaServerCP_EventCallback_ConnectionManager_SourceProtocolInfo != NULL)
								{
									MediaServerCP_EventCallback_ConnectionManager_SourceProtocolInfo(service,SourceProtocolInfo);
								}
							}
							if(xml->NameLength==16 && memcmp(xml->Name,"SinkProtocolInfo",16) == 0)
							{
								...
								if(MediaServerCP_EventCallback_ConnectionManager_SinkProtocolInfo != NULL)
								{
									MediaServerCP_EventCallback_ConnectionManager_SinkProtocolInfo(service,SinkProtocolInfo);
								}
							}
							if(xml->NameLength==20 && memcmp(xml->Name,"CurrentConnectionIDs",20) == 0)
							{
								...
								if(MediaServerCP_EventCallback_ConnectionManager_CurrentConnectionIDs != NULL)
								{
									MediaServerCP_EventCallback_ConnectionManager_CurrentConnectionIDs(service,CurrentConnectionIDs);
								}
							}
							if(xml->Peer!=NULL)
							{
...
void MediaServerCP_ContentDirectory_EventSink(char* buffer, int bufferlength, struct UPnPService *service)
{
	struct ILibXMLNode *xml,*rootXML;
	char *tempString;
	int tempStringLength;
					...
					if(xml->NameLength==8 && memcmp(xml->Name,"property",8)==0)
					{
						...
						while(flg2==0)
						{
							if(xml->NameLength==18 && memcmp(xml->Name,"ContainerUpdateIDs",18) == 0)
							{
								...
								if(MediaServerCP_EventCallback_ContentDirectory_ContainerUpdateIDs != NULL)
								{
									MediaServerCP_EventCallback_ContentDirectory_ContainerUpdateIDs(service,ContainerUpdateIDs);
								}
							}
							if(xml->NameLength==14 && memcmp(xml->Name,"SystemUpdateID",14) == 0)
							{
								...
								if(MediaServerCP_EventCallback_ContentDirectory_SystemUpdateID != NULL)
								{
									MediaServerCP_EventCallback_ContentDirectory_SystemUpdateID(service,SystemUpdateID);
								}
							}
							if(xml->Peer!=NULL)
							{
...
//
void MediaServerCP_OnEventSink(
...
int done)
{
	...
	if(done!=0)
	{
		...
		ILibAddHeaderLine(resp,"Server",6,MediaServerCP_PLATFORM,(int)strlen(MediaServerCP_PLATFORM));
		...
		while(field!=NULL)
		{
			if(field->FieldLength==3)
			{
				if(strncasecmp(field->Field,"SID",3)==0)
				{
					...
					//
					value = ILibGetEntry(((struct MediaServerCP_CP*)sender->User)->SIDTable,field->FieldData,field->FieldDataLength);
					break;
				}
			}
		...
		else
		{
			...
			{
				MediaServerCP_ConnectionManager_EventSink(buffer, BufferSize, service);
			}
			else
			if(type_length>46 && strncmp("urn:schemas-upnp-org:service:ContentDirectory:",service->ServiceType,46)==0)
			{
				MediaServerCP_ContentDirectory_EventSink(buffer, BufferSize, service);
			}
...
// has gone through
//
void MediaServerCP_OnUnSubscribeSink(
void *WebReaderToken,
int IsInterrupt,
struct packetheader *header,
char *buffer,
...
{
	struct UPnPService *s;
	//struct MediaServerCP_CP *cp = (struct MediaServerCP_CP*)vcp;
	...
	if(done!=0)
	{
		...
		MediaServerCP_Release(s->Parent);
	}
}
...
// has gone through
//
void MediaServerCP_OnSubscribeSink(
void *WebReaderToken,
int IsInterrupt,
struct packetheader *header,
char *buffer,
			...
			if(ILibWebClientIsStatusOk(header->StatusCode))
			{
				...
				while(field!=NULL)
				{
					...
					if(field->FieldLength==7 && strncasecmp(field->Field,"TIMEOUT",7)==0)
					{
						...
						MediaServerCP_AddRef(s->Parent);
						d = s->Parent;
						while(d->Parent!=NULL) {d = d->Parent;}
						++d->ReferenceTiedToEvents;
						ILibLifeTime_Add(cp->LifeTimeMonitor,s,atoi(p->LastResult->data)/2,(void*)&MediaServerCP_Renew,NULL);
...
void MediaServerCP_Renew(void *state)
{
	...
	ILibAddHeaderLine(p,"User-Agent",10,MediaServerCP_PLATFORM,(int)strlen(MediaServerCP_PLATFORM));
	...
	//
	ILibWebClient_PipelineRequest(
	((struct MediaServerCP_CP*)service->Parent->CP)->HTTP,
	&destaddr,
	p,
	&MediaServerCP_OnSubscribeSink,
...
struct UPnPDevice* MediaServerCP_GetDeviceEx(struct UPnPDevice *device, char* DeviceType, int counter, int number)
{
	struct UPnPDevice *RetVal = NULL;
	...
	{
		RetVal = MediaServerCP_GetDeviceEx(d,DeviceType,counter,number);
		d = d->Next;
	}
...
/*! \fn MediaServerCP_HasAction(struct UPnPService *s, char* action)
...
//
void MediaServerCP_StopCP(void *v_CP)
{
	int i;
	struct UPnPDevice *Device;
	struct MediaServerCP_CP *CP= (struct MediaServerCP_CP*)v_CP;
	...
	while(ILibHashTree_MoveNext(en)==0)
	{
		...
		if(data!=NULL)
		{
			...
			if(Device->ReservedID==0 && Device->Parent==NULL) // This is a UPnPDevice if ReservedID==0
			{
				...
				MediaServerCP_CP_RecursiveReleaseAndEventDevice(CP,Device);
				i = Device->ReferenceTiedToEvents;
				while(i!=0)
				{
					MediaServerCP_Release(Device);
...
{
	if(ILibIsChainBeingDestroyed(((struct MediaServerCP_CP*)CPToken)->Chain)==0)
	{
		((struct MediaServerCP_CP*)CPToken)->RecheckFlag = 1;
		ILibForceUnBlockChain(((struct MediaServerCP_CP*)CPToken)->Chain);
	}
}

void MediaServerCP_CP_PreSelect(void *CPToken,fd_set *readset, fd_set *writeset, fd_set *errorset, int *blocktime)
{
	struct MediaServerCP_CP *CP= (struct MediaServerCP_CP*)CPToken;
	void *en;
...
void MediaServerCP_OnSessionSink(struct ILibWebServer_Session *session, void *User)
{
	session->OnReceive = &MediaServerCP_OnEventSink;
	session->User = User;
}
...
void MediaServerCP_Debug(struct ILibWebServer_Session *sender, struct UPnPDevice *dv)
{
	char tmp[25];
...
void MediaServerCP_DebugOnQueryWCDO(struct ILibWebServer_Session *sender, char *w)
{
	struct MediaServerCP_CP *cp = (struct MediaServerCP_CP*)sender->User3;
...
void MediaServerCP_DebugOnQuery(struct ILibWebServer_Session *sender, char *UDN, char *URI, char *TOK)
{
	struct MediaServerCP_CP *cp = (struct MediaServerCP_CP*)sender->User3;
	struct packetheader *p = ILibCreateEmptyPacket();
	void *en;
...
void MediaServerCP_OnDebugReceive(struct ILibWebServer_Session *sender, int InterruptFlag, struct packetheader *header, char *bodyBuffer, int *beginPointer, int endPointer, int done)
{
	...
	if(stricmp(header->Directive,"GET")==0)
	{
		...
		{
			MediaServerCP_DebugOnQuery(sender,header->DirectiveObj+5,NULL,NULL);
		}
		else if(memcmp(header->DirectiveObj,"/URI/",5)==0)
		{
			MediaServerCP_DebugOnQuery(sender,NULL,header->DirectiveObj+5,NULL);
		}
		else if(memcmp(header->DirectiveObj,"/TOK/",5)==0)
		{
			MediaServerCP_DebugOnQuery(sender,NULL,NULL,header->DirectiveObj+5);
		}
		else if(memcmp(header->DirectiveObj,"/WCDO/",6)==0)
		{
			MediaServerCP_DebugOnQueryWCDO(sender,header->DirectiveObj+6);
		}
		else
		{
...
void MediaServerCP_OnDebugSessionSink(struct ILibWebServer_Session *sender, void *user)
{
	sender->OnReceive = &MediaServerCP_OnDebugReceive;
	sender->User3 = user;
}
#endif
/*! \fn MediaServerCP_CreateControlPoint(void *Chain, void(*A)(struct UPnPDevice*),void(*R)(struct UPnPDevice*))
...
{
	struct MediaServerCP_CP *cp = (struct MediaServerCP_CP*)malloc(sizeof(struct MediaServerCP_CP));
	
	cp->Destroy = &MediaServerCP_StopCP;
	cp->PostSelect = NULL;
	cp->PreSelect = &MediaServerCP_CP_PreSelect;
	cp->DiscoverSink = A;
	cp->RemoveSink = R;
	...
	#ifdef UPNP_DEBUG
	ILibWebServer_Create(Chain,2,7575,&MediaServerCP_OnDebugSessionSink,cp);
	#endif
	
	cp->SSDP = ILibCreateSSDPClientModule(Chain,"urn:schemas-upnp-org:device:MediaServer:1", 41, &MediaServerCP_SSDP_Sink,cp);
...
void MediaServerCP_Invoke_ConnectionManager_GetCurrentConnectionIDs_Sink(
void *WebReaderToken,
int IsInterrupt,
struct packetheader *header,
char *buffer,
...
int *PAUSE)
{
	...
	{
		MediaServerCP_Release(Service->Parent);
		free(_InvokeData);
		return;
	}
	else
	{
		...
		{
			/* SOAP Fault */
			((void (*)(struct UPnPService*,int,void*,char*))_InvokeData->CallbackPtr)(Service,MediaServerCP_GetErrorCode(buffer,EndPointer-(*p_BeginPointer)),_InvokeData->User,INVALID_DATA);
			MediaServerCP_Release(Service->Parent);
			free(_InvokeData);
			return;
		}
...
void MediaServerCP_Invoke_ConnectionManager_GetCurrentConnectionInfo_Sink(
void *WebReaderToken,
int IsInterrupt,
struct packetheader *header,
char *buffer,
...
int *PAUSE)
{
	...
	else
	{
		...
		{
			/* SOAP Fault */
			((void (*)(struct UPnPService*,int,void*,int,int,char*,char*,int,char*,char*))_InvokeData->CallbackPtr)(Service,MediaServerCP_GetErrorCode(buffer,EndPointer-(*p_BeginPointer)),_InvokeData->User,INVALID_DATA,INVALID_DATA,INVALID_DATA,INVALID_DATA,INVALID_DATA,INVALID_DATA,INVALID_DATA);
			MediaServerCP_Release(Service->Parent);
			free(_InvokeData);
			return;
		}
...
void MediaServerCP_Invoke_ConnectionManager_GetProtocolInfo_Sink(
void *WebReaderToken,
int IsInterrupt,
struct packetheader *header,
char *buffer,
...
int *PAUSE)
{
	...
	else
	{
		...
		{
			/* SOAP Fault */
			((void (*)(struct UPnPService*,int,void*,char*,char*))_InvokeData->CallbackPtr)(Service,MediaServerCP_GetErrorCode(buffer,EndPointer-(*p_BeginPointer)),_InvokeData->User,INVALID_DATA,INVALID_DATA);
			MediaServerCP_Release(Service->Parent);
			free(_InvokeData);
			return;
		}
...
void MediaServerCP_Invoke_ContentDirectory_Browse_Sink(
void *WebReaderToken,
int IsInterrupt,
struct packetheader *header,
char *buffer,
...
int *PAUSE)
{
	...
	else
	{
		...
		{
			/* SOAP Fault */
			((void (*)(struct UPnPService*,int,void*,char*,unsigned int,unsigned int,unsigned int))_InvokeData->CallbackPtr)(Service,MediaServerCP_GetErrorCode(buffer,EndPointer-(*p_BeginPointer)),_InvokeData->User,INVALID_DATA,INVALID_DATA,INVALID_DATA,INVALID_DATA);
			MediaServerCP_Release(Service->Parent);
			free(_InvokeData);
			return;
		}
...
void MediaServerCP_Invoke_ContentDirectory_CreateObject_Sink(
void *WebReaderToken,
int IsInterrupt,
struct packetheader *header,
char *buffer,
...
void MediaServerCP_Invoke_ContentDirectory_DestroyObject_Sink(
void *WebReaderToken,
int IsInterrupt,
struct packetheader *header,
char *buffer,
...
int *PAUSE)
{
	...
	else
	{
		...
		{
			/* SOAP Fault */
			((void (*)(struct UPnPService*,int,void*))_InvokeData->CallbackPtr)(Service,MediaServerCP_GetErrorCode(buffer,EndPointer-(*p_BeginPointer)),_InvokeData->User);
			MediaServerCP_Release(Service->Parent);
			free(_InvokeData);
			return;
		}
...
void MediaServerCP_Invoke_ContentDirectory_GetSearchCapabilities_Sink(
void *WebReaderToken,
int IsInterrupt,
struct packetheader *header,
char *buffer,
...
void MediaServerCP_Invoke_ContentDirectory_GetSortCapabilities_Sink(
void *WebReaderToken,
int IsInterrupt,
struct packetheader *header,
char *buffer,
...
void MediaServerCP_Invoke_ContentDirectory_GetSystemUpdateID_Sink(
void *WebReaderToken,
int IsInterrupt,
struct packetheader *header,
char *buffer,
...
int *PAUSE)
{
	...
	else
	{
		...
		{
			/* SOAP Fault */
			((void (*)(struct UPnPService*,int,void*,unsigned int))_InvokeData->CallbackPtr)(Service,MediaServerCP_GetErrorCode(buffer,EndPointer-(*p_BeginPointer)),_InvokeData->User,INVALID_DATA);
			MediaServerCP_Release(Service->Parent);
			free(_InvokeData);
			return;
		}
...
void MediaServerCP_Invoke_ContentDirectory_Search_Sink(
void *WebReaderToken,
int IsInterrupt,
struct packetheader *header,
char *buffer,
...
/*! \fn MediaServerCP_Invoke_ConnectionManager_GetCurrentConnectionIDs(struct UPnPService *service,void (*CallbackPtr)(struct UPnPService *sender,int ErrorCode,void *user,char* ConnectionIDs), void *_user)
...
*/
void MediaServerCP_Invoke_ConnectionManager_GetCurrentConnectionIDs(struct UPnPService *service,void (*CallbackPtr)(struct UPnPService *sender,int ErrorCode,void *user,char* ConnectionIDs), void* user)
{
	int headerLength;
	char *headerBuffer;
	...
	MediaServerCP_AddRef(service->Parent);
	ILibParseUri(service->ControlURL,&IP,&Port,&Path);
	
	headerBuffer = (char*)malloc(188 + (int)strlen(MediaServerCP_PLATFORM) + (int)strlen(Path) + (int)strlen(IP) + (int)strlen(service->ServiceType));
	headerLength = sprintf(headerBuffer,UPNPCP_SOAP_Header,Path,IP,Port,MediaServerCP_PLATFORM,service->ServiceType,"GetCurrentConnectionIDs",bufferLength);
	...
	bufferLength,
	0,
	&MediaServerCP_Invoke_ConnectionManager_GetCurrentConnectionIDs_Sink,
	service,
	invoke_data);
...
void MediaServerCP_Invoke_ConnectionManager_GetCurrentConnectionInfo(struct UPnPService *service,void (*CallbackPtr)(struct UPnPService *sender,int ErrorCode,void *user,int RcsID,int AVTransportID,char* ProtocolInfo,char* PeerConnectionManager,int PeerConnectionID,char* Direction,char* Status), void* user, int ConnectionID)
{
	...
	headerBuffer = (char*)malloc(189 + (int)strlen(MediaServerCP_PLATFORM) + (int)strlen(Path) + (int)strlen(IP) + (int)strlen(service->ServiceType));
	headerLength = sprintf(headerBuffer,UPNPCP_SOAP_Header,Path,IP,Port,MediaServerCP_PLATFORM,service->ServiceType,"GetCurrentConnectionInfo",bufferLength);
	...
	bufferLength,
	0,
	&MediaServerCP_Invoke_ConnectionManager_GetCurrentConnectionInfo_Sink,
	service,
	invoke_data);
...
/*! \fn MediaServerCP_Invoke_ConnectionManager_GetProtocolInfo(struct UPnPService *service,void (*CallbackPtr)(struct UPnPService *sender,int ErrorCode,void *user,char* Source,char* Sink), void *_user)
\brief Invokes the GetProtocolInfo action in the ConnectionManager service
\param service The UPnPService instance to invoke the action on
\param CallbackPtr The function pointer to be called when the invocation returns
*/
void MediaServerCP_Invoke_ConnectionManager_GetProtocolInfo(struct UPnPService *service,void (*CallbackPtr)(struct UPnPService *sender,int ErrorCode,void *user,char* Source,char* Sink), void* user)
{
	int headerLength;
	char *headerBuffer;
	char *SoapBodyTemplate;
	...
	headerBuffer = (char*)malloc(180 + (int)strlen(MediaServerCP_PLATFORM) + (int)strlen(Path) + (int)strlen(IP) + (int)strlen(service->ServiceType));
	headerLength = sprintf(headerBuffer,UPNPCP_SOAP_Header,Path,IP,Port,MediaServerCP_PLATFORM,service->ServiceType,"GetProtocolInfo",bufferLength);
	...
	bufferLength,
	0,
	&MediaServerCP_Invoke_ConnectionManager_GetProtocolInfo_Sink,
	service,
	invoke_data);
...
void MediaServerCP_Invoke_ContentDirectory_Browse(struct UPnPService *service,void (*CallbackPtr)(struct UPnPService *sender,int ErrorCode,void *user,char* Result,unsigned int NumberReturned,unsigned int TotalMatches,unsigned int UpdateID), void* user, char* ObjectID, char* BrowseFlag, char* Filter, unsigned int StartingIndex, unsigned int RequestedCount, char* SortCriteria)
{
	...
	headerBuffer = (char*)malloc(171 + (int)strlen(MediaServerCP_PLATFORM) + (int)strlen(Path) + (int)strlen(IP) + (int)strlen(service->ServiceType));
	headerLength = sprintf(headerBuffer,UPNPCP_SOAP_Header,Path,IP,Port,MediaServerCP_PLATFORM,service->ServiceType,"Browse",bufferLength);
	...
	bufferLength,
	0,
	&MediaServerCP_Invoke_ContentDirectory_Browse_Sink,
	service,
	invoke_data);
...
/*! \fn MediaServerCP_Invoke_ContentDirectory_CreateObject(struct UPnPService *service,void (*CallbackPtr)(struct UPnPService *sender,int ErrorCode,void *user,char* ObjectID,char* Result), void *_user, char* ContainerID, char* Elements)
...
*/
void MediaServerCP_Invoke_ContentDirectory_CreateObject(struct UPnPService *service,void (*CallbackPtr)(struct UPnPService *sender,int ErrorCode,void *user,char* ObjectID,char* Result), void* user, char* ContainerID, char* Elements)
{
	int headerLength;
	char *headerBuffer;
	char *SoapBodyTemplate;
	...
	headerBuffer = (char*)malloc(177 + (int)strlen(MediaServerCP_PLATFORM) + (int)strlen(Path) + (int)strlen(IP) + (int)strlen(service->ServiceType));
	headerLength = sprintf(headerBuffer,UPNPCP_SOAP_Header,Path,IP,Port,MediaServerCP_PLATFORM,service->ServiceType,"CreateObject",bufferLength);
	...
	bufferLength,
	0,
	&MediaServerCP_Invoke_ContentDirectory_CreateObject_Sink,
	service,
	invoke_data);
...
/*! \fn MediaServerCP_Invoke_ContentDirectory_DestroyObject(struct UPnPService *service,void (*CallbackPtr)(struct UPnPService *sender,int ErrorCode,void *user), void *_user, char* unescaped_ObjectID)
...
*/
void MediaServerCP_Invoke_ContentDirectory_DestroyObject(struct UPnPService *service,void (*CallbackPtr)(struct UPnPService *sender,int ErrorCode,void *user), void* user, char* unescaped_ObjectID)
{
	int headerLength;
	char *headerBuffer;
	char *SoapBodyTemplate;
	...
	headerBuffer = (char*)malloc(178 + (int)strlen(MediaServerCP_PLATFORM) + (int)strlen(Path) + (int)strlen(IP) + (int)strlen(service->ServiceType));
	headerLength = sprintf(headerBuffer,UPNPCP_SOAP_Header,Path,IP,Port,MediaServerCP_PLATFORM,service->ServiceType,"DestroyObject",bufferLength);
	...
	bufferLength,
	0,
	&MediaServerCP_Invoke_ContentDirectory_DestroyObject_Sink,
	service,
	invoke_data);
...
/*! \fn MediaServerCP_Invoke_ContentDirectory_GetSearchCapabilities(struct UPnPService *service,void (*CallbackPtr)(struct UPnPService *sender,int ErrorCode,void *user,char* SearchCaps), void *_user)
\brief Invokes the GetSearchCapabilities action in the ContentDirectory service
\param service The UPnPService instance to invoke the action on
\param CallbackPtr The function pointer to be called when the invocation returns
*/
void MediaServerCP_Invoke_ContentDirectory_GetSearchCapabilities(struct UPnPService *service,void (*CallbackPtr)(struct UPnPService *sender,int ErrorCode,void *user,char* SearchCaps), void* user)
{
	int headerLength;
	char *headerBuffer;
	char *SoapBodyTemplate;
	...
	headerBuffer = (char*)malloc(186 + (int)strlen(MediaServerCP_PLATFORM) + (int)strlen(Path) + (int)strlen(IP) + (int)strlen(service->ServiceType));
	headerLength = sprintf(headerBuffer,UPNPCP_SOAP_Header,Path,IP,Port,MediaServerCP_PLATFORM,service->ServiceType,"GetSearchCapabilities",bufferLength);
	...
	bufferLength,
	0,
	&MediaServerCP_Invoke_ContentDirectory_GetSearchCapabilities_Sink,
	service,
	invoke_data);
...
/*! \fn MediaServerCP_Invoke_ContentDirectory_GetSortCapabilities(struct UPnPService *service,void (*CallbackPtr)(struct UPnPService *sender,int ErrorCode,void *user,char* SortCaps), void *_user)
\brief Invokes the GetSortCapabilities action in the ContentDirectory service
\param service The UPnPService instance to invoke the action on
\param CallbackPtr The function pointer to be called when the invocation returns
*/
void MediaServerCP_Invoke_ContentDirectory_GetSortCapabilities(struct UPnPService *service,void (*CallbackPtr)(struct UPnPService *sender,int ErrorCode,void *user,char* SortCaps), void* user)
{
	int headerLength;
	char *headerBuffer;
	char *SoapBodyTemplate;
	...
	headerBuffer = (char*)malloc(184 + (int)strlen(MediaServerCP_PLATFORM) + (int)strlen(Path) + (int)strlen(IP) + (int)strlen(service->ServiceType));
	headerLength = sprintf(headerBuffer,UPNPCP_SOAP_Header,Path,IP,Port,MediaServerCP_PLATFORM,service->ServiceType,"GetSortCapabilities",bufferLength);
	...
	bufferLength,
	0,
	&MediaServerCP_Invoke_ContentDirectory_GetSortCapabilities_Sink,
	service,
	invoke_data);
...
/*! \fn MediaServerCP_Invoke_ContentDirectory_GetSystemUpdateID(struct UPnPService *service,void (*CallbackPtr)(struct UPnPService *sender,int ErrorCode,void *user,unsigned int Id), void *_user)
\brief Invokes the GetSystemUpdateID action in the ContentDirectory service
\param service The UPnPService instance to invoke the action on
\param CallbackPtr The function pointer to be called when the invocation returns
*/
void MediaServerCP_Invoke_ContentDirectory_GetSystemUpdateID(struct UPnPService *service,void (*CallbackPtr)(struct UPnPService *sender,int ErrorCode,void *user,unsigned int Id), void* user)
{
	int headerLength;
	char *headerBuffer;
	char *SoapBodyTemplate;
	...
	headerBuffer = (char*)malloc(182 + (int)strlen(MediaServerCP_PLATFORM) + (int)strlen(Path) + (int)strlen(IP) + (int)strlen(service->ServiceType));
	headerLength = sprintf(headerBuffer,UPNPCP_SOAP_Header,Path,IP,Port,MediaServerCP_PLATFORM,service->ServiceType,"GetSystemUpdateID",bufferLength);
	...
	bufferLength,
	0,
	&MediaServerCP_Invoke_ContentDirectory_GetSystemUpdateID_Sink,
	service,
	invoke_data);
...
void MediaServerCP_Invoke_ContentDirectory_Search(struct UPnPService *service,void (*CallbackPtr)(struct UPnPService *sender,int ErrorCode,void *user,char* Result,unsigned int NumberReturned,unsigned int TotalMatches,unsigned int UpdateID), void* user, char* ContainerID, char* SearchCriteria, char* Filter, unsigned int StartingIndex, unsigned int RequestedCount, char* SortCriteria)
{
	...
	headerBuffer = (char*)malloc(171 + (int)strlen(MediaServerCP_PLATFORM) + (int)strlen(Path) + (int)strlen(IP) + (int)strlen(service->ServiceType));
	headerLength = sprintf(headerBuffer,UPNPCP_SOAP_Header,Path,IP,Port,MediaServerCP_PLATFORM,service->ServiceType,"Search",bufferLength);
	...
	bufferLength,
	0,
	&MediaServerCP_Invoke_ContentDirectory_Search_Sink,
	service,
	invoke_data);
...
/*! \fn MediaServerCP_UnSubscribeUPnPEvents(struct UPnPService *service)
...
void MediaServerCP_UnSubscribeUPnPEvents(struct UPnPService *service)
{
	...
	&destaddr,
	p,
	&MediaServerCP_OnUnSubscribeSink,
	(void*)service,
	service->Parent->CP);
...
/*! \fn MediaServerCP_SubscribeForUPnPEvents(struct UPnPService *service, void(*callbackPtr)(struct UPnPService* service,int OK))
...
void MediaServerCP_SubscribeForUPnPEvents(struct UPnPService *service, void(*callbackPtr)(struct UPnPService* service,int OK))
{
	...
	sprintf(callback,"<http://%s:%d%s>",service->Parent->InterfaceToHost,ILibWebServer_GetPortNumber(((struct MediaServerCP_CP*)service->Parent->CP)->WebServer),Path);
...
/*! \fn MediaServerCP_GetCustomXML_X_DLNACAP(struct UPnPDevice *d)
...
{
	return(MediaServerCP_GetCustomTagFromDevice(d,X_DLNACAP_NAMESPACE, X_DLNACAP));
}
/*! \fn MediaServerCP_GetCustomXML_X_DLNADOC(struct UPnPDevice *d)
...
{
	return(MediaServerCP_GetCustomTagFromDevice(d,X_DLNADOC_NAMESPACE, X_DLNADOC));
}
