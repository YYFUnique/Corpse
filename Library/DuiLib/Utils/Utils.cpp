...
namespace DuiLib
{
	...
	void CDuiImage::CreateEmptyImage(int nWidth,int nHeight,int nbb)
	{
	...
	INT_PTR CDuiArray::GetSize() const
	{ 
		return m_nSize; 
	}

	INT_PTR CDuiArray::GetCount() const
	{
		return m_nSize; 
	}

	const CString* CDuiArray::GetData() const
	{ 
		return (const CString*)m_pData; 
	}

	CString* CDuiArray::GetData()
	{
		return (CString*)m_pData; 
	}

	INT_PTR CDuiArray::Add(LPCTSTR newElement)
	{ 
	...
	INT_PTR CDuiArray::Add(const CString& newElement)
	{
		INT_PTR nIndex = m_nSize;
		SetAtGrow(nIndex, newElement);
		return nIndex; 
	...
	INT_PTR CDuiArray::Append(const CDuiArray& src)
	{
	...
	void CDuiArray::Copy(const CDuiArray& src)
	{
		ASSERT(this != &src);   // cannot append to itself
	...
	BOOL CDuiArray::IsEmpty() const
	{ 
		return m_nSize == 0; 
	}

	void CDuiArray::RemoveAll()
	{ 
		SetSize(0);
	}

	DWORD CDuiArray::Find(LPCTSTR lpszItem)
	{
	...
	const CString& CDuiArray::GetAt(INT_PTR nIndex) const
	{ 
		ASSERT(nIndex >= 0 && nIndex < m_nSize);
		return m_pData[nIndex]; 
	}

	void CDuiArray::SetAt(INT_PTR nIndex, LPCTSTR newElement)
	{ 
		ASSERT(nIndex >= 0 && nIndex < m_nSize);
		m_pData[nIndex] = newElement; 
	}

	void CDuiArray::SetAt(INT_PTR nIndex, const CString& newElement)
	{ 
		ASSERT(nIndex >= 0 && nIndex < m_nSize);
		m_pData[nIndex] = newElement;
	}

	CString& CDuiArray::ElementAt(INT_PTR nIndex)
	{
		ASSERT(nIndex >= 0 && nIndex < m_nSize);
		return m_pData[nIndex]; 
	}

	void CDuiArray::SetAtGrow(INT_PTR nIndex, LPCTSTR newElement)
	{
		ASSERT(nIndex >= 0);
	...
	void CDuiArray::SetSize(INT_PTR nNewSize, INT_PTR nGrowBy)
	{
		ASSERT(nNewSize >= 0);
	...
	void CDuiArray::SetAtGrow(INT_PTR nIndex, const CString& newElement)
	{
		ASSERT(this);
		ASSERT(nIndex >= 0);
	...
	void CDuiArray::InsertAt(INT_PTR nIndex, LPCTSTR newElement, INT_PTR nCount)
	{
	...
	void CDuiArray::InsertAt(INT_PTR nIndex, const CString& newElement, INT_PTR nCount)
	{
		// make room for new elements
		InsertEmpty(nIndex, nCount);
		// copy elements into the empty space
	...
	void CDuiArray::RemoveAt(INT_PTR nIndex, INT_PTR nCount)
	{
		ASSERT(nIndex >= 0);
		ASSERT(nCount >= 0);
	...
	const CString& CDuiArray::operator[](INT_PTR nIndex) const
	{ 
		return GetAt(nIndex); 
	}

	CString& CDuiArray::operator[](INT_PTR nIndex)
	{ 
		return ElementAt(nIndex); 
	}


	void CDuiArray::InsertEmpty(INT_PTR nIndex, INT_PTR nCount)
	{
    ...
    int CDuiString::Format(LPCTSTR pstrFormat, va_list Args)
    {
        ...
        nLen = _vsctprintf(pstrFormat,Args);
	...
	LPVOID CStdStringPtrMap::Find(UINT nId,bool optimize/* = true*/) const
	{
		TCHAR szKey[20];
		_stprintf(szKey,_T("%u"),nId);
		return Find(szKey,optimize);
	}

	bool CStdStringPtrMap::Insert(UINT nId,LPVOID pData)
	{
		TCHAR szKey[20];
		_stprintf(szKey,_T("%u"),nId);
		return Insert(szKey,pData);
	}

	LPVOID CStdStringPtrMap::Set(UINT nId,LPVOID pData)
	{
		TCHAR szKey[20];
		_stprintf(szKey,_T("%u"),nId);
		return Set(szKey,pData);
	}
	...
	bool CStdStringPtrMap::Remove(UINT nId)
	{
		TCHAR szKey[20];
		_stprintf(szKey,_T("%u"),nId);
		return Remove(szKey);
