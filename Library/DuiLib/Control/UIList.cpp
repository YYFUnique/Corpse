...
void CListHeaderUI::SetPos(RECT rc)
{
	...
	for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
		...
		if (m_bIsScaleHeader && ((pHeaderItem->GetButtonState() & UISTATE_CAPTURED) != UISTATE_CAPTURED))
		{
		...
		else
		{
			...
			if (pHeaderItem->GetScale() != 0)
				pHeaderItem->SetScale(sz.cx*100/GetWidth());
		}
...
UINT CListHeaderItemUI::GetButtonState()
{
