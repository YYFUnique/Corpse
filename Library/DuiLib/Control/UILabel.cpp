...
namespace DuiLib
{
	...
	void CLabelUI::SetShadowOffset( int _offset,int _angle )
	{
		try
		{
			if(_angle > 180 || _angle < -180)
				return;
			m_ShadowOffset.X = (float)(_offset*cos(_angle*3.1415926/180));
			m_ShadowOffset.Y = (float)(_offset*sin(_angle*3.1415926/180));
			Invalidate();
		}
		catch (...)
		{
	...
	void CLabelUI::SetMultiLine(bool _MultiLine)
	{
		if(_MultiLine){
