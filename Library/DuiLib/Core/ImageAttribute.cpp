...
namespace DuiLib
{
	...
	void CImageAttribute::Clone(const CImageAttribute& image)
	{
		...
		m_pImageInfo = image.m_pImageInfo;
	...
	void CImageAttribute::SetAttributeString(LPCTSTR pStrImageAttr)
	{
		if (m_sImageAttribute == pStrImageAttr)
			return;

		Clear();

		m_sImageAttribute = pStrImageAttr;
		//m_sImage = m_sImageAttribute;

		ParseAttribute(pStrImageAttr);
	}
	...
	bool CImageAttribute::LoadImage(CPaintManagerUI* pManager)
	{
		if (m_pImageInfo != NULL)
			return true;
		if (m_bLoadSuccess == FALSE)
			return false;

		const TImageInfo* pData = NULL;
		if (m_sResType.IsEmpty())
			pData = pManager->GetImageEx((LPCTSTR)m_sImage, NULL, m_dwMask);
		else
			pData = pManager->GetImageEx((LPCTSTR)m_sImage, (LPCTSTR)m_sResType, m_dwMask);
		...
		{
			m_rcSource.right = pData->nX;
			m_rcSource.bottom = pData->nY;
		}

		if (m_rcSource.right > pData->nX) m_rcSource.right = pData->nX;
		if (m_rcSource.bottom > pData->nY) m_rcSource.bottom = pData->nY;
		m_pImageInfo = const_cast<TImageInfo*>(pData);
	...
	LPCTSTR CImageAttribute::GetImage() const
	{
		return m_sImage;
	}

	LPCTSTR CImageAttribute::GetResType() const
	{
		return m_sResType;
	}

	TImageInfo* CImageAttribute::GetImageInfo() const
	{
		return m_pImageInfo;
	}

	RECT CImageAttribute::GetDestRect() const
	{
		return m_rcDest;
	}

	RECT CImageAttribute::GetCornerRect() const
	{
		return m_rcCorner;
	}

	RECT CImageAttribute::GetSourceRect() const
	{
		return m_rcSource;
	}

	BYTE CImageAttribute::GetFade() const
	{
		return m_bFade;
	}

	DWORD CImageAttribute::GetMask() const
	{
		return m_dwMask;
	}

	bool CImageAttribute::IsHole() const
	{
		return m_bHole;
	}

	bool CImageAttribute::IsTiledX() const
	{
		return m_bTiledX;
	}

	bool CImageAttribute::IsTiledY() const
	{
		return m_bTiledY;
	}
}