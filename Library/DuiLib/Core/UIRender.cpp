...
HBITMAP CreateAlphaTextBitmap(LPCTSTR inText, HFONT inFont, COLORREF inColour,RECT TextArea, UINT format)
{ 
...
{
	HBITMAP MyBMP = CreateAlphaTextBitmap(szText, hFont, dwColor,rc, format);
	...
	if (MyBMP)
	{
		...
		if (hOldBMP)
		{
			...
			if (format & DT_RIGHT)
				nLeft += rc.right - rc.left - BMInf.bmWidth;
			else if (format & DT_CENTER)
				nLeft += (rc.right - rc.left - BMInf.bmWidth) / 2;

			if (format & DT_BOTTOM)
				nTop += rc.bottom - rc.bottom - BMInf.bmHeight;
			else if (format & DT_VCENTER)
				nTop += (rc.bottom - rc.top - BMInf.bmHeight) /2;

			AlphaBlend(inDC, nLeft, nTop, BMInf.bmWidth, BMInf.bmHeight, hTempDC, 0, 0, BMInf.bmWidth, BMInf.bmHeight, bf); 
...
BOOL CRenderEngine::SaveBitmapFile(HBITMAP hBitmap,LPCTSTR lpszFileName)
{
	HDC hDC;         
	//设备描述表
	int iBits;
...
TImageInfo* CRenderEngine::LoadImage(LPBYTE pData, DWORD dwSize, DWORD mask)
{
	LPBYTE pImage = NULL;
	int x,y,n;
	pImage = stbi_load_from_memory(pData, dwSize, &x, &y, &n, 4);
    ...
    for( int i = 0; i < 2; ++i,image_count = 0 ) {
        ...
        while( *pStrImage != _T('\0') ) {
            ...
            if( !sValue.IsEmpty() ) {
				...
				else if( sItem == _T("dst")){
					rcItem.left = rc.left + _tcstol(sValue.GetData(), &pstr, 10);  ASSERT(pstr);    
					rcItem.top = rc.top + _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr);
					rcItem.right = rc.right - _tcstol(pstr + 1, &pstr, 10);  ASSERT(pstr);
					if (rcItem.right < rcItem.left)	 rcItem.right = rcItem.left;
					rcItem.bottom = rc.bottom - _tcstol(pstr + 1, &pstr, 10); ASSERT(pstr);
...
void CRenderEngine::DrawColor(HDC hDC, const RECT& rc, DWORD color,SIZE borderRound,long bTopLeftRound/*=1*/,long bTopRightRound/*=1*/,long bBottomRightRound/*=1*/,long bBottomLeftRound/*=1*/)
{
#ifdef RENDER_GDIPLUS
	Graphics graphics(hDC);
	
	if(borderRound.cx>0&&borderRound.cy>0)//圆角度
	{
		graphics.SetSmoothingMode(SmoothingModeAntiAlias);
		CGraphicsRoundRectPath roundPath(rc.left,rc.top,rc.right-rc.left-1,rc.bottom-rc.top-1,borderRound.cx,borderRound.cy,bTopLeftRound,bTopRightRound,bBottomRightRound,bBottomLeftRound);
		graphics.FillPath(&SolidBrush(Color(color)),&roundPath);
		graphics.SetSmoothingMode(SmoothingModeDefault);
	}
	else
	{
		graphics.FillRectangle(&SolidBrush(Color(color)),Rect(rc.left,rc.top,rc.right-rc.left,rc.bottom-rc.top));
	}
#else
...
void CRenderEngine::DrawPolygon(HDC hDC,POINT * pPoints,int nNumber,DWORD dwFillColor,DWORD dwBorderColor/*=0*/,int nMode/*=0*/)//0 Fill 1:border 2:border and fill
{
	ASSERT(nNumber>0);
#ifdef RENDER_GDIPLUS
	Graphics graphic(hDC);
	Point * points=new Point[nNumber];
	for(int i=0;i<nNumber;i++)
	{
		points[i]=Point(pPoints[i].x,pPoints[i].y);
	}
	
	if(nMode==0||nMode==2)
	{
		graphic.SetSmoothingMode(SmoothingModeHighQuality);
		graphic.FillPolygon(&SolidBrush(Color(dwFillColor)),points,nNumber,FillModeAlternate);
		graphic.SetSmoothingMode(SmoothingModeDefault);
	}
	if(nMode==1||nMode==2)
	{
		graphic.SetSmoothingMode(SmoothingModeHighQuality);
		graphic.DrawPolygon(&Pen(Color(dwBorderColor)),points,nNumber);
		graphic.SetSmoothingMode(SmoothingModeDefault);
	}
	delete [] points;
#else
	...
	if(nMode==0||nMode==2)
	{
		...
		PolyDraw(hDC,pPoints,NULL,nNumber);
	}
	if(nMode==1||nMode==2)
	{
...
void CRenderEngine::Draw3dRect(HDC hDC,const RECT& rect,DWORD clrTopLeft, DWORD clrBottomRight)
{
	RECT rectLeft={rect.left, rect.top, rect.left+1, rect.bottom};
	RECT rectTop={rect.left, rect.top, rect.right, rect.top+1};
	RECT rectRight={rect.right-1,rect.top,rect.right,rect.bottom};
	RECT rectBottom={rect.left,rect.bottom-1,rect.right,rect.bottom};
	
	DrawColor(hDC,rectLeft,clrTopLeft);
	DrawColor(hDC,rectTop,clrTopLeft);
	DrawColor(hDC,rectRight,clrBottomRight);
...
void CRenderEngine::DrawGradient(HDC hDC, const RECT& rc, DWORD *dwColors, float*fPositions, int colorCount,bool bVertical,SIZE borderRound,long bTopLeftRound/*=1*/,long bTopRightRound/*=1*/,long bBottomRightRound/*=1*/,long bBottomLeftRound/*=1*/)
{
	if(colorCount<=0)
		return;
	...
	{
		colors[i]=Color(dwColors[i]);
	}
	LinearGradientBrush linGrBrush(bVertical?Point(0,rc.top):Point(rc.left,0),bVertical?Point(0,rc.bottom):Point(rc.right,0),Color(255,255,0,0),Color(255,0,0,255)); 
	linGrBrush.SetInterpolationColors(colors,fPositions,colorCount);
	...
	if(borderRound.cx>0&&borderRound.cy>0)//圆角度
	{
		graphics.SetSmoothingMode(SmoothingModeAntiAlias);
		CGraphicsRoundRectPath roundPath(rc.left,rc.top,rc.right-rc.left-1,rc.bottom-rc.top-1,borderRound.cx,borderRound.cy,bTopLeftRound,bTopRightRound,bBottomRightRound,bBottomLeftRound);
		graphics.FillPath(&linGrBrush,&roundPath);
		graphics.SetSmoothingMode(SmoothingModeDefault);
	}
	else
	{
		graphics.FillRectangle(&linGrBrush,Rect(rc.left,rc.top,rc.right-rc.left,rc.bottom-rc.top));
	}
	delete colors;
}
