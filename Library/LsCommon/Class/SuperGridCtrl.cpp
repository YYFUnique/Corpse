...
CSuperGridCtrl::CSuperGridCtrl()
{
	...
	m_psPlusMinus.CreatePen(PS_SOLID, 1, RGB(192,192,192));
...
{
	cs.style |= LVS_REPORT | LVS_SINGLESEL | LVS_SHAREIMAGELISTS | LVS_OWNERDRAWFIXED;	
...
int CSuperGridCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	//SetWindowPos(0,-1,-1,lpc,cy,SWP_SHOWWINDOW);
	return 0;
}
...
void CSuperGridCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	if(lpDrawItemStruct->itemAction == ODA_DRAWENTIRE)
	{
		...
		COLORREF clf = pItemInfo->GetItemColor();
		if(clf!=-1)
		{	
			CBrush br(clf);
...
CTreeItem* CSuperGridCtrl::InsertItem(CTreeItem *pParent, CItemInfo* lpInfo,  BOOL bUpdate)
{
	if(pParent==NULL)
		return NULL;
...
CTreeItem* CSuperGridCtrl::GetRootItem(int nIndex)
{
...
CTreeItem*  CSuperGridCtrl::InsertRootItem(CItemInfo * lpInfo)
{
	if(lpInfo==NULL)
		lpInfo = new CItemInfo;
...
CTreeItem* CSuperGridCtrl::GetNext(CTreeItem* pStartAt, CTreeItem* pNode, BOOL bInit, BOOL bDontIncludeHidden)
{
	static BOOL bFound;
	if (bInit)
		bFound = FALSE;
...
CTreeItem* CSuperGridCtrl::GetPrev(CTreeItem* pStartAt, CTreeItem* pNode, BOOL bInit, BOOL bDontIncludeHidden)
{
...
CTreeItem* CSuperGridCtrl::GetNextRoot(POSITION& pos) const
{
	return (CTreeItem*)m_RootItems.GetNext(pos);
}


CTreeItem* CSuperGridCtrl::GetPrevRoot(POSITION& pos) const
{
	return (CTreeItem*)m_RootItems.GetNext(pos);
}
...
CTreeItem* CSuperGridCtrl::GetNextChild(CTreeItem *pItem, POSITION& pos) const
{
...
CTreeItem* CSuperGridCtrl::GetPrevChild(CTreeItem *pItem, POSITION& pos) const
{
...
CTreeItem* CSuperGridCtrl::GetParentItem(const CTreeItem* pItem) 
{
...
CTreeItem* CSuperGridCtrl::GetTreeItem(int nIndex /*nIndex must be valid of course*/ ) 
{
