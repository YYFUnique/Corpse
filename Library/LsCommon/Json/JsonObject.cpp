...
void CJsonObject::AllValueToUtf8()
{
	EnumJsonMemberSetValueEncoding(this,JSON_UTF8);
}


void CJsonObject::AllValueToGb23()
{
	EnumJsonMemberSetValueEncoding(this,JSON_GB2312);
}

void  CJsonObject::EnumJsonMemberSetValueEncoding(CJsonObject * lpJson,UCHAR chEncodingType)
{
	CJsonMemberNameList			cJsonMemNameList;
	char*						pTempText = NULL;
	POSITION					positionJson=NULL;
	CString						strChildValue;
	CJsonObject					cJsonEnumObj;
	CJsonObject					cJsonItem;

	lpJson->EnumJsonMemberNames(cJsonMemNameList );//枚举对象
	positionJson=cJsonMemNameList.GetHeadPosition();
	while(positionJson)
	{
		ENUM_JSON_MEMBER_NAME &enumJsonMember=cJsonMemNameList.GetNext(positionJson);

		if(enumJsonMember.enumType==objectValue )
		{
			lpJson->GetValue(enumJsonMember.strMemberName, (CJsonObject *)&cJsonEnumObj);
			EnumJsonMemberSetValueEncoding(&cJsonEnumObj,chEncodingType);
			lpJson->SetValue(enumJsonMember.strMemberName, (CJsonObject *)&cJsonEnumObj);
		}
		else if(enumJsonMember.enumType==arrayValue)
		{
			lpJson->GetValue(enumJsonMember.strMemberName, (CJsonObject *)&cJsonEnumObj);
			int nSize=cJsonEnumObj.GetArraySize();
			for(int nBegin=0;nBegin<cJsonEnumObj.GetArraySize();++nBegin)
			{
				cJsonItem=cJsonEnumObj.GetArrayItem(nBegin);
				EnumJsonMemberSetValueEncoding(&cJsonItem,chEncodingType);
				cJsonEnumObj.SetArrayItem(nBegin,&cJsonItem);
			}
		}
		else if(enumJsonMember.enumType==stringValue)
		{
			strChildValue= lpJson->GetValue(enumJsonMember.strMemberName);

			if(chEncodingType==JSON_GB2312)
				strChildValue=Utf8ToGb32(strChildValue);

			else if(chEncodingType==JSON_UTF8)
				strChildValue=Gb32ToUtf8(strChildValue);
			
			lpJson->SetValue(enumJsonMember.strMemberName, strChildValue);
		}
	}
}
...
void CJsonObject::SetValue(LPCTSTR key, const CJsonObjectArray *value)
{
	if(value)
	{
		CStringA strKey(key);
		CJsonObject JsonObjectTemp;
		JsonObjectTemp.FromString(value->ToString());
...
void CJsonObject::GetSubValueToArray(LPCTSTR key,CStringArray& arrSubItemString)
{
	arrSubItemString.RemoveAll();

	CJsonObject SubJson ;
	GetValue(key, &SubJson);
	int nSubItemCount=SubJson.GetArraySize();
	CString strTemp = SubJson.ToString();

	CJsonObject JsonObjectTemp;
	for(int i=0;i<nSubItemCount;i++)
	{
		JsonObjectTemp=SubJson.GetArrayItem(i);

		CString strItemText = JsonObjectTemp.GetValue();;

		arrSubItemString.Add(strItemText);
	}
}

BOOL CJsonObject::GetSubValueToArray(LPCTSTR key,CMap<DWORD,DWORD,DWORD,DWORD>& IpHostMap)
{
	IpHostMap.RemoveAll();
	
	CStringArray arrstrIpArray;
	GetSubValueToArray(key,arrstrIpArray);
	
	int nItemCount = (int)arrstrIpArray.GetSize();
	
	for(int i=0;i<nItemCount;i++)
	{
		CStringA strIP(arrstrIpArray.ElementAt(i));
		DWORD dwIp = inet_addr(strIP);
		IpHostMap.SetAt(dwIp, 0);
	}
	
	return TRUE;
}

BOOL CJsonObject::GetSubValueToArray(LPCTSTR key,CDWordArray& dwIpArray)
{
	dwIpArray.RemoveAll();
...
CJsonObjectArray::CJsonObjectArray()
{
}
...
int CJsonObjectArray::Add(const CJsonObject &json)
{
	return (int)m_arrJsons.Add(json);
}

CJsonObject& CJsonObjectArray::GetAt(int nIndex)
{
	return m_arrJsons.ElementAt(nIndex);
}

BOOL CJsonObjectArray::FromString(CString& strJson)
{
	CJsonObject Json;
	strJson.TrimLeft(_T("["));
	...
	if (strJson.IsEmpty())
	{
		SetErrorInfo(CUSTOM_ERROR,0,_T("不能从空串中加载Json对象"));
		return FALSE;
	}
	DWORD dwOffset = 0;
	LPCTSTR lpszJson = strJson;
	LPCTSTR lpszBlock = NULL;
	while((lpszBlock = lpszJson+dwOffset))
	{
		if (*lpszBlock == NULL)
			break;
		if (Json.FromString(lpszBlock) == FALSE)
		{
			SetErrorInfo(CUSTOM_ERROR,0,_T("从字符串加载Json对象失败"));
			return FALSE;
		}
		m_arrJsons.Add(Json);
		dwOffset += Json.ToRawString().GetLength()+1;
	}

	return TRUE;
}

void CJsonObjectArray::Clear()
{
	m_arrJsons.RemoveAll();
}

int CJsonObjectArray::GetArraySize()
{
	return (int)m_arrJsons.GetSize();
}

CString CJsonObjectArray::ToString()
{
	CString strResult(_T('['));
	
	int nCount = (int)m_arrJsons.GetSize();
	for(int i = 0; i < nCount; i++)
	{
		CString strTemp(m_arrJsons.ElementAt(i).ToString());
		if(!strTemp.IsEmpty())
		{
			if((strResult + strTemp).GetLength() >= MAX_REPORT_INFO_SIZE)
				break;
...
CString CJsonObjectArray::ToString() const
{
	CString strResult(_T('['));
	...
	for(int i = 0; i < nCount; i++)
	{
		CString strTemp(m_arrJsons[i].ToString());
		if(!strTemp.IsEmpty())
		{
			if((strResult + strTemp).GetLength() >= MAX_REPORT_INFO_SIZE)
				break;
...
CString CJsonObjectArray::ToRawString() const
{
	CString strResult(_T('['));
	...
	for(int i = 0; i < nCount; i++)
	{
		CString strTemp(m_arrJsons[i].ToRawString());
		if(!strTemp.IsEmpty())
		{
			if((strResult + strTemp).GetLength() >= MAX_REPORT_INFO_SIZE)
				break;
