...
class LS_COMMON_API CRijndael : public CEncryptObject
{
public:
	//Operation Modes
...
public:
	...
	virtual void SetKey(const void* lpKey, const void* lpChain, int nKeyLength=DEFAULT_BLOCK_SIZE, int nBlockSize=DEFAULT_BLOCK_SIZE);

private:
	//Auxiliary Function
	void Xor(BYTE* buff, BYTE const* chain)
	{
		for(DWORD i=0; i<m_blockSize; i++)
			*(buff++) ^= *(chain++);	
	}
	...
	void DefEncryptBlock(BYTE const* in, BYTE* result);

	//Convenience method to decrypt exactly one block of plaintext, assuming
	//Rijndael's default block size (128-bit).
	// in         - The ciphertext.
	// result     - The plaintext generated from a ciphertext using the session key.
	void DefDecryptBlock(BYTE const* in, BYTE* result);

public:
	//Encrypt exactly one block of plaintext.
	// in           - The plaintext.
    // result       - The ciphertext generated from a plaintext using the key.
    void EncryptBlock(BYTE const* in, BYTE* result);
	
	//Decrypt exactly one block of ciphertext.
	// in         - The ciphertext.
	// result     - The plaintext generated from a ciphertext using the session key.
	void DecryptBlock(BYTE const* in, BYTE* result);

	virtual BOOL Encrypt(const void* in, void* result, DWORD dwBufferLen, int iMode=ECB);
	
	virtual BOOL Decrypt(const void* in, void* result, DWORD dwBufferLen, int iMode=ECB);
