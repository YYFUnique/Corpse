...
{
	CStringA strIP(lpszAddress);
	IN_ADDR InAddr;
	InAddr.s_addr = inet_addr(strIP);
	CString strIPAddr(inet_ntoa(InAddr));

	return strIPAddr.CompareNoCase(lpszAddress) == 0;
	/*if(!lpszAddress)
		return FALSE;
	...
	for(int i = 0; i < len; ++i)
	{
		...
		else if(c == '.' || c == '\0')
		{
			...
			TCHAR szSub[4] = {0};
			_tcscpy_s(szSub, iSubLen , lpszAddress + iDotIndex);
...
{
	CONNID dwConnID	= ::InterlockedIncrement((LONG *)&dwSeed);
	
	if(dwConnID == 0)
		dwConnID = ::InterlockedIncrement((LONG *)&dwSeed);
