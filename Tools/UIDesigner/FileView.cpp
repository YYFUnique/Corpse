...
BEGIN_MESSAGE_MAP(CFileView, CDockablePane)
	...
	ON_WM_DESTROY()
	ON_COMMAND(ID_ADD_EXIST_FILE, &CFileView::OnAddExistFile)
	ON_COMMAND(ID_MENU_PACK_CONFIGURE, &CFileView::OnPackConfigure)
END_MESSAGE_MAP()
...
void CFileView::OnContextMenu(CWnd* pWnd, CPoint point)
{
	...
	CContextMenuManager* pMenuManager = theApp.GetContextMenuManager();
	if (point != CPoint(-1, -1))
	{
		// 选择已单击的项:
		CPoint ptTree = point;
	...
	pWndTree->SetFocus();
	pMenuManager->ShowPopupMenu(IDR_POPUP_EXPLORER, point.x, point.y, this, TRUE);
}
...
CDocument* CFileView::GetDocument(CRuntimeClass* pRuntimeClass)
{
	CDocTemplate* pDocTemplate = theApp.GetUIDocTemplate();
	POSITION posDoc = pDocTemplate->GetFirstDocPosition();
	while (posDoc != NULL)
	{
		CDocument* pDoc = pDocTemplate->GetNextDoc(posDoc);
		if (pDoc->IsKindOf(pRuntimeClass))
			return pDoc;
	}
...
void CFileView::OnAddExistFile()
{
	// TODO: 在此添加命令处理程序代码
	CFileDialog FileDialog(TRUE,_T("XML"),NULL,OFN_HIDEREADONLY,_T("xml文件(*.xml)|*.xml||"));
	if (FileDialog.DoModal() == IDOK)
	{
		HTREEITEM hSelectedItem = m_wndFileView.GetSelectedItem();
		DWORD dwInfo = m_wndFileView.GetItemData(hSelectedItem);
		HTREEITEM hParent = (dwInfo != INFO_FILE) ? hSelectedItem : m_wndFileView.GetParentItem(hSelectedItem);
		HTREEITEM hNewItem = m_wndFileView.InsertItem(FileDialog.GetFileName(), 2, 2, hParent);
		m_wndFileView.SetItemData(hNewItem, INFO_FILE);
		m_wndFileView.Expand(hParent, TVE_EXPAND);
		OpenSkinFile(hNewItem);
	}
}

BOOL CFileView::GetProjectResourceFile(CStringArray& strResourceFile)
{
	BOOL bSuccess = FALSE;

	do 
	{
		CStringArray strSkinFile;
		if (g_pResourceView->GetMapTree(strSkinFile) != FALSE)
		{
			for (INT i=0;i<strSkinFile.GetSize();++i)
			{
				const CStringArray* pstrImageFile = g_pResourceView->GetImageTree(strSkinFile.GetAt(i));
				if (pstrImageFile == NULL)
					break;

				CString strSkinPath;
				g_pResourceView->GetSkinFilePath(strSkinFile.GetAt(i),strSkinPath);
				strResourceFile.Add(strSkinPath);
				for (INT j=0;j<pstrImageFile->GetSize();++j)
				{
					strResourceFile.Add(pstrImageFile->GetAt(j));
				}
			}
		}
		bSuccess = TRUE;
...
void CFileView::OnPackConfigure()
{
// 	HZIP hZip = NULL;
// 	do 
// 	{
// 		LPCTSTR lpszZipFileOutPath = CGlobalVariable::m_strProjectPath;
// 		LPCTSTR lpszProjectPath = CGlobalVariable::m_strProjectPath;
// 		CString strPackFileName;
// 		strPackFileName.Format(_T("%s.zip"),PathFindFileName(CGlobalVariable::m_strProjectName));
// 
// 		TCHAR szZipFileOutPath[MAX_PATH];
// 		PathCombine(szZipFileOutPath,lpszZipFileOutPath,strPackFileName);
// 		hZip = CreateZip(szZipFileOutPath,0,ZIP_FILENAME);
// 		if (hZip == NULL)
// 			break;
// 
// 		CStringArray strResourceFile;
// 		GetProjectResourceFile(strResourceFile);
// 		TCHAR szFilePath[MAX_PATH];
// 		for (INT i=0;i<strResourceFile.GetSize();++i)
// 		{
// 			_tcscpy_s(szFilePath,strResourceFile.ElementAt(i));
// 			if (ZipAdd(hZip,PathFindFileName(szFilePath),szFilePath,0,ZIP_FILENAME) != S_OK)
// 				break;
// 		}
// 
// 		CloseZip(hZip);
