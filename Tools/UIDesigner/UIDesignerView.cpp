...
CLayoutManager* CUIDesignerView::GetLayoutManager()
{
...
{
	g_pPropertiesWnd->ShowProperty(m_MultiTracker.GetFocused());
	CString strSkinDir = m_LayoutManager.GetSkinDir();
	g_HookAPI.SetSkinDir(strSkinDir);
	m_LayoutManager.GetManager()->SetCurrentPath(strSkinDir);
}
...
void CUIDesignerView::PasteUI(LPCTSTR xml)
{
	...
	CControlUI* pRoot=builder.Create(xml, (UINT)0, this, m_LayoutManager.GetManager());
	if(pRoot)
	{
...
CControlUI* CUIDesignerView::CreateControl(LPCTSTR pstrClass)
{
	CControlUI* pControl = NULL;
// 	if (_tcsicmp(pstrClass,_T("ListCtrl")) == 0)
// 		pControl = new CListCtrlUI;
	return pControl;
}