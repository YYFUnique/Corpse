...
CControlUI* CControlExtend::CreateControl(LPCTSTR pstrClass)
{
	...
	CString strControlUI;
	strControlUI.Format(_T("%s.xml"),pstrClass);
	CControlUI* pControlUI = DialogExtend.Create((LPCTSTR)strControlUI); 
	return pControlUI;
}

CView* CControlExtend::GetView(CRuntimeClass* RunTimeClass)
{
...
bool CFormUI::IsBackgroundTransparent()
{
...
void CFormUI::SetAlpha( int nOpacity )
{
	m_nOpacity=nOpacity;
...
void CLayoutManager::Init(HWND hWnd,LPCTSTR pstrLoad)
{
	...
	if(*pstrLoad!='\0')
	{
		...
		CControlExtend DialogCallback;
		CControlUI* pRoot=builder.Create(pstrLoad,(UINT)0,&DialogCallback,&m_Manager);
		if(pRoot)
			m_pFormUI->Add(pRoot);
...
CControlUI* CLayoutManager::NewUI(int nClass,CRect& rect,CControlUI* pParent, CLayoutManager* pLayout)
{
	...
	case classActiveX:
		...
		break;
	case classVScrollBar:
		pControl=new CScrollBarUI;
		pExtended->nClass=classVScrollBar;
	case classContainer:
...
void CLayoutManager::SaveOptionProperty(CControlUI* pControl, TiXmlElement* pNode)
{
	...
	if (pOptionUI->GetSelectedPressImage() && _tcslen(pOptionUI->GetSelectedPressImage()) > 0)
		pNode->SetAttribute("selectedpressimage",StringConvertor::WideToUtf8(ConvertImageFileName(pOptionUI->GetSelectedPressImage())));
}
...
void CLayoutManager::SaveComboProperty(CControlUI* pControl, TiXmlElement* pNode)
{
	...
	DWORD dwFont = pComboUI->GetItemFont();
	if (dwFont != -1)
	{
		_stprintf_s(szBuf,_T("%d"),dwFont);
		pNode->SetAttribute("font",StringConvertor::WideToUtf8(szBuf));
	}

	CRect Rect = pComboUI->GetTextPadding();

	if (Rect.IsRectNull() == FALSE)
	{
		_stprintf_s(szBuf,_T("%d,%d,%d,%d"),Rect.left,Rect.top,Rect.right,Rect.bottom);
		pNode->SetAttribute("textpadding",StringConvertor::WideToUtf8(szBuf));
	}
...
void CLayoutManager::SaveActiveXProperty(CControlUI* pControl, TiXmlElement* pNode)
{
	...
	if (IsEqualGUID(pActiveX->GetClisd(),GUID_NULL) == FALSE)
	{
		LPTSTR lpszClsid = NULL;
		if (SUCCEEDED(StringFromCLSID(pActiveX->GetClisd(),&lpszClsid)))
		{
			CString strActivexCList(lpszClsid);
			if (_tcsicmp(strActivexCList.Left(1),_T("{")))
 				strActivexCList.Insert(0,_T("{"));
 			if (_tcsicmp(strActivexCList.Right(1),_T("}")))
 				strActivexCList += _T("}");
 			pNode->SetAttribute("clsid", StringConvertor::WideToUtf8(strActivexCList));
...
void CLayoutManager::SaveTabLayoutProperty(CControlUI* pControl, TiXmlElement* pNode)
{
	...
	LONG nSelectedId = pTabLayout->GetCurSel();
	if(nSelectedId)
	{
		_stprintf_s(szBuf, _T("%d"),nSelectedId);
		pNode->SetAttribute("selectedid", StringConvertor::WideToUtf8(szBuf));
	}
}

void CLayoutManager::SaveListLabelElementProperty(CControlUI* pControl,TiXmlElement* pNode)
{
	ASSERT(pControl);
	SaveControlProperty(pControl, pNode);
	TCHAR szBuf[MAX_PATH] = {0};
	CListLabelElementUI* pListLabelElementUI = static_cast<CListLabelElementUI*>(pControl->GetInterface(_T("ListLabelElement")));
	if (pListLabelElementUI->IsSelected())
	{
		_tcscpy(szBuf,_T("true"));
		pNode->SetAttribute("selected",StringConvertor::WideToUtf8(szBuf));
	}
}
									...
									, BOOL bSaveChildren/* = TRUE*/)
{
	...
	case classTileLayout:
		...
		break;
	case classListLabelElement:
		SaveListLabelElementProperty(pControl,pNode);
		break;
	default:
		/*CDefaultControl* pDefaultContrl = (CDefaultControl*)pControl;
		if (pDefaultContrl->SaveDefaultControlProperty(pNode) == FALSE)
		{
...
void CLayoutManager::SaveSkinFile(LPCTSTR pstrPathName)
{
	...
	HANDLE hFile = ::CreateFile(pstrPathName, GENERIC_ALL, FILE_SHARE_WRITE|FILE_SHARE_READ, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	bool bTransparent = pForm->IsBackgroundTransparent();
	if (bTransparent)
	{
		_tcscpy(szBuf,_T("true"));
