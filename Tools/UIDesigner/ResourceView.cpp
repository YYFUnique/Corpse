...
void CResourceViewBar::InsertImageTree(LPCTSTR lpszTitle, CString strPath)
{
	POSITION pos = m_TreeItem.AddTail(strPath);
	HTREEITEM hRoot = m_wndResourceView.InsertItem(TVIF_PARAM|LVIF_TEXT|TVIF_STATE,lpszTitle,0,0,
							TVIS_BOLD,TVIS_BOLD,(LPARAM)pos,TVI_ROOT,NULL);
	//m_wndResourceView.SetItemState(hRoot, TVIS_BOLD, TVIS_BOLD);
	m_mapTree.SetAt(lpszTitle, (void*)hRoot);
	
	CStringArray* pstrArray = new CStringArray;
	m_mapImageArray.SetAt(lpszTitle, (void*)pstrArray);
	...
	if(nPos == -1)
		return;
	CString strDirectory = strPath.Left(nPos + 1);

	WIN32_FIND_DATA FindFileData = {0};
	CString strFind = strDirectory + _T("*.*");
	...
	do
	{
		...
		if(nType > 0)
		{
			pstrArray->Add(strDirectory + FindFileData.cFileName);
...
void CResourceViewBar::RemoveImageTree(const CString& strTree)
{
	HTREEITEM hTree;
...
void CResourceViewBar::InsertImage(const CString& strImage, const CString& strTree)
{
	if(strImage.IsEmpty())
		return;
	HTREEITEM hTree;
...
const CStringArray* CResourceViewBar::GetImageTree(const CString& strTree) const
{
	CStringArray* pstrArray = NULL;
...
BOOL CResourceViewBar::GetMapTree(CStringArray& strMapTree)
{
	strMapTree.RemoveAll();
	POSITION pos = m_mapTree.GetStartPosition();
	CString strKey;
	CStringArray* pstrArray;
	while(pos)
	{
		m_mapTree.GetNextAssoc(pos,strKey,(LPVOID&)pstrArray);
		strMapTree.Add(strKey);
	}
	return TRUE;
}

BOOL CResourceViewBar::GetSkinFilePath(LPCTSTR lpszTreeName,CString& strTreeSkinPath)
{
	HTREEITEM hTreeItem;
	if (m_mapTree.Lookup(lpszTreeName,(LPVOID&)hTreeItem) == FALSE)
		return FALSE;
	
	POSITION pos = (POSITION)m_wndResourceView.GetItemData(hTreeItem);
	strTreeSkinPath = m_TreeItem.GetAt(pos);
	return TRUE;
}
