...
void CUIProperties::InitPropList()
{
	...
	pProp=new CMFCPropertyGridProperty(_T("MaxBox"),(_variant_t)true,_T("指示窗口能否最大化"),tagMaxBox);
	pPropUI->AddSubItem(pProp);
	...
	//tagBkTrans
	pProp=new CMFCPropertyGridProperty(_T("Bktrans"),(_variant_t)true,_T("是否使用背景透明"),tagTransparent);
	pPropUI->AddSubItem(pProp);
	...
	pProp=new CMFCPropertyGridProperty(_T("ToolTip"),(_variant_t)_T(""),_T("控件提示文字"),tagToolTip);//ToolTip
	pPropUI->AddSubItem(pProp);
	...
	m_wndPropList.AddProperty(pPropUI);
 	...
 	pProp=new CMFCPropertyGridProperty(_T("VScrollBar"),(_variant_t)false,_T("是否具有垂直滚动条"),tagRichEditVscrollbar);//VScrollBar
 	pPropUI->AddSubItem(pProp);
 
 	pProp=new CMFCPropertyGridProperty(_T("HScrollBar"),(_variant_t)false,_T("是否具有水平滚动条"),tagRichEditHscrollbar);//HScrollBar
 	pPropUI->AddSubItem(pProp);
 
 	pProp=new CMFCPropertyGridProperty(_T("WantTab"),(_variant_t)false,_T("是否可以获取Tab"),tagRichEditWanttab);//WantTab
 	pPropUI->AddSubItem(pProp);
 
 	pProp=new CMFCPropertyGridProperty(_T("Return"),(_variant_t)false,_T("是否具有换行"),tagRichEditWantreturn);//Return
 	pPropUI->AddSubItem(pProp);
 
 	pProp=new CMFCPropertyGridProperty(_T("Multiline"),(_variant_t)false,_T("是否可以多行显示"),tagRichEditMultiline);//Multiline
 	pPropUI->AddSubItem(pProp);
 
 	pProp=new CMFCPropertyGridProperty(_T("Readonly"),(_variant_t)false,_T("指示是否只读"),tagRichEditReadonly);//Readonly
 	pPropUI->AddSubItem(pProp);
 
 	pProp=new CMFCPropertyGridProperty(_T("Password"),(_variant_t)false,_T("指示是否使用密码框"),tagRichEditPassword);//password
 	pPropUI->AddSubItem(pProp);
	...
	pPropImage=new CMFCPropertyGridImageProperty(_T("ForeImage"),_T(""),_T("复选框前景图片"),tagForeImage);
	pPropImage->AllowEdit(FALSE);
	pPropUI->AddSubItem(pPropImage);
	...
	pPropImage=new CMFCPropertyGridImageProperty(_T("SelectedHotImage"),_T(""),_T("指定复选框被选择后鼠标悬浮图片"),tagSelectedHotImage);//selectedimage
	pPropImage->AllowEdit(FALSE);
	pPropUI->AddSubItem(pPropImage);

	pPropImage=new CMFCPropertyGridImageProperty(_T("SelectedPressImage"),_T(""),_T("指定复选框被选择后按下图片"),tagSelectedPressImage);//selectedimage
	pPropImage->AllowEdit(FALSE);
	pPropUI->AddSubItem(pPropImage);
	...
	//ListLableElement
	pPropUI = new CMFCPropertyGridProperty(_T("ListLabelElement"),classListLabelElement);
	pProp=new CMFCPropertyGridProperty(_T("height"),(_variant_t)(LONG)0,_T("列表项高度"),tagLabelHeight);
	pPropUI->AddSubItem(pProp);
	pProp=new CMFCPropertyGridProperty(_T("selected"),(_variant_t)false,_T("选中"),tagLabelSelected);
	pPropUI->AddSubItem(pProp);
	m_wndPropList.AddProperty(pPropUI);

	//TabLayout
	pPropUI = new CMFCPropertyGridProperty(_T("Tablayout"),classTabLayout);
	pProp = new CMFCPropertyGridProperty(_T("SelectedId"),(_variant_t)(LONG)0,_T("选择项序号"),tagSelectId);
	pPropUI->AddSubItem(pProp);
	m_wndPropList.AddProperty(pPropUI);

	//VScrollBar
//  	pPropUI = new CMFCPropertyGridProperty(_T("VScrollBar"),classVScrollBar);
//  	pProp = new CMFCPropertyGridProperty(_T("ShowButton1"),(_variant_t)(BOOL)TRUE,_T("是否显示按钮1"),tagShowButton1);
//  	pPropUI->AddSubItem(pProp);
//  	pProp = new CMFCPropertyGridProperty(_T("ShowButton2"),(_variant_t)(BOOL)TRUE,_T("是否显示按钮2"),tagShowButton2);
//  	pPropUI->AddSubItem(pProp);
//  	pPropUI = new CMFCPropertyGridProperty(_T(""));
...
void CUIProperties::ShowProperty(CControlUI* pControl)
{
	...
	case classDialogLayout:
		...
		break;
	case classTabLayout:
		ShowTabLayoutProperty(pControl);
		break;
...
void CUIProperties::ShowFormProperty(CControlUI* pControl)
{
	...
	// tagAlpha
	pPropForm->GetSubItem(tagAlpha-tagForm)->SetValue((_variant_t)(LONG)pForm->GetAlpha());
	pPropForm->GetSubItem(tagAlpha-tagForm)->SetOriginalValue((_variant_t)(LONG)pForm->GetAlpha());

	// tagBkTrans
	pPropForm->GetSubItem(tagTransparent-tagForm)->SetValue((_variant_t)pForm->IsBackgroundTransparent());
	pPropForm->GetSubItem(tagTransparent-tagForm)->SetOriginalValue((_variant_t)pForm->IsBackgroundTransparent());

	// tagDefaultFontColor
	static_cast<CMFCPropertyGridColor32Property*>(pPropForm->GetSubItem(tagDefaultFontColor-tagForm))->SetColor((_variant_t)(LONG)(pForm->GetDefaultFontColor()));
	static_cast<CMFCPropertyGridColor32Property*>(pPropForm->GetSubItem(tagDefaultFontColor-tagForm))->SetOriginalValue((_variant_t)(LONG)(pForm->GetDefaultFontColor()));

	// tagSelectedFontColor
	static_cast<CMFCPropertyGridColor32Property*>(pPropForm->GetSubItem(tagSelectedFontColor-tagForm))->SetColor((_variant_t)(LONG)(pForm->GetDefaultSelectedFontColor()));
	static_cast<CMFCPropertyGridColor32Property*>(pPropForm->GetSubItem(tagSelectedFontColor-tagForm))->SetOriginalValue((_variant_t)(LONG)(pForm->GetDefaultSelectedFontColor()));

	// tagDisabledFontColor
	static_cast<CMFCPropertyGridColor32Property*>(pPropForm->GetSubItem(tagDisabledFontColor-tagForm))->SetColor((_variant_t)(LONG)(pForm->GetDefaultDisabledFontColor()));
	static_cast<CMFCPropertyGridColor32Property*>(pPropForm->GetSubItem(tagDisabledFontColor-tagForm))->SetOriginalValue((_variant_t)(LONG)(pForm->GetDefaultDisabledFontColor()));

	// tagLinkFontColor
	static_cast<CMFCPropertyGridColor32Property*>(pPropForm->GetSubItem(tagLinkFontColor-tagForm))->SetColor((_variant_t)(LONG)(pForm->GetDefaultLinkFontColor()));
	static_cast<CMFCPropertyGridColor32Property*>(pPropForm->GetSubItem(tagLinkFontColor-tagForm))->SetOriginalValue((_variant_t)(LONG)(pForm->GetDefaultLinkFontColor()));

	// tagLinkHoverFontColor
	static_cast<CMFCPropertyGridColor32Property*>(pPropForm->GetSubItem(tagLinkHoverFontColor-tagForm))->SetColor((_variant_t)(LONG)(pForm->GetDefaultLinkHoverFontColor()));
	static_cast<CMFCPropertyGridColor32Property*>(pPropForm->GetSubItem(tagLinkHoverFontColor-tagForm))->SetOriginalValue((_variant_t)(LONG)(pForm->GetDefaultLinkHoverFontColor()));
...
void CUIProperties::ShowRichEditProperty(CControlUI* pControl)
{
	...
	//readonly
	pPropRichEdit->GetSubItem(tagRichEditReadonly-tagRichEdit)->SetValue((_variant_t)pRichEdit->IsReadOnly());
	pPropRichEdit->GetSubItem(tagRichEditReadonly-tagRichEdit)->SetOriginalValue((_variant_t)pRichEdit->IsReadOnly());

// 	//readonly
// 	pPropEdit->GetSubItem(tagRichEditMultiline-tagRichEdit)->SetValue((_variant_t)pRichEdit->IsReadOnly());
// 	pPropEdit->GetSubItem(tagRichEditMultiline-tagRichEdit)->SetOriginalValue((_variant_t)pRichEdit->IsReadOnly());
// 
// 	//readonly
// 	pPropEdit->GetSubItem(tagRichEditVscrollbar-tagRichEdit)->SetValue((_variant_t)pRichEdit);
// 	pPropEdit->GetSubItem(tagRichEditVscrollbar-tagRichEdit)->SetOriginalValue((_variant_t)pRichEdit->IsReadOnly());
// 	//password
// 	pPropEdit->GetSubItem(tagPassword-tagEdit)->SetValue((_variant_t)pRichEdit->IsPasswordMode());
// 	pPropEdit->GetSubItem(tagPassword-tagEdit)->SetOriginalValue((_variant_t)pRichEdit->IsPasswordMode());
...
void CUIProperties::ShowOptionProperty(CControlUI* pControl)
{
	...
	//ForeImage
	pPropOption->GetSubItem(tagForeImage-tagOption)->SetValue((_variant_t)pOption->GetForeImage());
	pPropOption->GetSubItem(tagForeImage-tagOption)->SetOriginalValue((_variant_t)pOption->GetForeImage());
	//selectedimage
	...
	//selectedpressimage
	pPropOption->GetSubItem(tagSelectedPressImage-tagOption)->SetValue((_variant_t)pOption->GetSelectedPressImage());
	pPropOption->GetSubItem(tagSelectedPressImage-tagOption)->SetOriginalValue((_variant_t)pOption->GetSelectedPressImage());
	//selected
...
void CUIProperties::ShowComboProperty(CControlUI* pControl)
{
	...
	//itemfont
	pPropCombo->GetSubItem(tagItemFont-tagCombo)->SetValue((_variant_t)(LONG)pCombo->GetItemFont());
	pPropCombo->GetSubItem(tagItemFont-tagCombo)->SetOriginalValue((_variant_t)(LONG)pCombo->GetItemFont());
	//itemshowhtml
...
void CUIProperties::ShowTabLayoutProperty(CControlUI* pControl)
{
	ShowContainerProperty(pControl);
	...
	CMFCPropertyGridProperty* pPropTabLayout=m_wndPropList.FindItemByData(classTabLayout,FALSE);
	ASSERT(pPropTabLayout);

	pPropTabLayout->GetSubItem(tagTabLayout-tagSelectId)->SetValue((_variant_t)(LONG)pTabLayout->GetCurSel());
	pPropTabLayout->GetSubItem(tagTabLayout-tagSelectId)->SetOriginalValue((_variant_t)(LONG)pTabLayout->GetCurSel());

	pPropTabLayout->Show(TRUE,FALSE);
}
...
void CUIProperties::ShowListLabelElement(CControlUI* pControl)
{
	ShowControlProperty(pControl);
	ASSERT(pControl);
	CListLabelElementUI* pListLabelElement=static_cast<CListLabelElementUI*>(pControl->GetInterface(_T("ListLabelElement")));
	ASSERT(pListLabelElement);

	CMFCPropertyGridProperty* pPropListLabelElement=m_wndPropList.FindItemByData(classListLabelElement,FALSE);
	ASSERT(pPropListLabelElement);
	
	//项内容
	pPropListLabelElement->GetSubItem(tagLabelHeight - tagListLabelElement)->SetValue((_variant_t)(LONG)pListLabelElement->GetHeight());
	pPropListLabelElement->GetSubItem(tagLabelHeight - tagListLabelElement)->SetOriginalValue((_variant_t)(LONG)pListLabelElement->GetHeight());

	//是否选中
	pPropListLabelElement->GetSubItem(tagLabelSelected - tagListLabelElement)->SetValue((_variant_t)pListLabelElement->IsSelected());
	pPropListLabelElement->GetSubItem(tagLabelSelected - tagListLabelElement)->SetOriginalValue((_variant_t)pListLabelElement->IsSelected());
	
	pPropListLabelElement->Show(TRUE,FALSE);
}
