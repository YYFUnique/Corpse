...
void CUITracker::Construct()
{
	...
	if (!bInitialized)
	{
		...
		m_hCursors[0] = ::LoadCursor(NULL, IDC_SIZENWSE);//西北-东南方向箭头 ATL_MAKEINTRESOURCE(AFX_IDC_TRACKNWSE));
		m_hCursors[1] = ::LoadCursor(NULL, IDC_SIZENESW);//东北-西南 (AFX_IDC_TRACKNESW));
		m_hCursors[2] = m_hCursors[0];
		m_hCursors[3] = m_hCursors[1];
		m_hCursors[4] = ::LoadCursor(NULL, IDC_SIZENS);//南北 (AFX_IDC_TRACKNS));
		m_hCursors[5] = ::LoadCursor(NULL, IDC_SIZEWE);//东西 (AFX_IDC_TRACKWE));
		m_hCursors[6] = m_hCursors[4];
		m_hCursors[7] = m_hCursors[5];
		m_hCursors[8] = ::LoadCursor(NULL, IDC_SIZEALL);//四方 (AFX_IDC_TRACK4WAY));
