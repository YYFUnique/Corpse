...
#define WM_ADD_USERDATA (WM_USER+0x1000)	// 发送自定义添加USERDATA消息

WinImplBase::WinImplBase()
{
...
void WinImplBase::Init(HWND hWnd)
{
...
LRESULT WinImplBase::__WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	WinImplBase* pThis = NULL;
	if( uMsg == WM_NCCREATE ) {
		LPCREATESTRUCT lpcs = reinterpret_cast<LPCREATESTRUCT>(lParam);
		pThis = static_cast<WinImplBase*>(lpcs->lpCreateParams);
		pThis->m_hWnd = hWnd;
		::SetWindowLongPtr(hWnd, GWLP_USERDATA, reinterpret_cast<LPARAM>(pThis));
	} 
	else {
		pThis = reinterpret_cast<WinImplBase*>(::GetWindowLongPtr(hWnd, GWLP_USERDATA));
