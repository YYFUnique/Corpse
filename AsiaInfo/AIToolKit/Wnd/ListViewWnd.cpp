...
void CListViewWnd::InitWindow()
{
	...
	// 重新计算窗口初始大小，并且应用该大小
	m_PaintManager.SetInitSize(m_pOwner->GetFixedWidth(), 300);
	::SetWindowPos(m_hWnd, NULL, 0, 0, m_pOwner->GetFixedWidth(), 300, SWP_NOZORDER | SWP_NOMOVE | SWP_NOACTIVATE);
...
void CListViewWnd::Add(const CString& strSupportKernel)
{
	CListContainerElementUI* pFileItem = NULL;
	if (m_DialogBuilder.GetMarkup()->IsValid() == false)
		pFileItem = (CListContainerElementUI*)m_DialogBuilder.Create(_T("SupportKernelItem.xml"), 0, NULL, &m_PaintManager);
	else
		pFileItem = (CListContainerElementUI*)m_DialogBuilder.Create(NULL, &m_PaintManager);

	m_pSearch->Add(pFileItem);
	CLabelUI* pKernelName = (CLabelUI*)pFileItem->FindSubControl(_T("searchitem_text"));
	pKernelName->SetText(strSupportKernel);
	pKernelName->SetToolTip(strSupportKernel);
}
